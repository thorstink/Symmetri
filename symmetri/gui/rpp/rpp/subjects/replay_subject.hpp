...
class replay_subject_base {
  struct replay_state final : public subject_state<Type, Serialized> {
    replay_state(size_t limit = std::numeric_limits<size_t>::max(),
                 rpp::schedulers::duration duration_limit =
                     std::numeric_limits<rpp::schedulers::duration>::max())
        : m_limit(limit), m_duration_limit(duration_limit) {}

    void add_value(const Type& v) {
      std::unique_lock lock{m_values_mutex};
      while (m_values.size() >= m_limit) m_values.pop_front();

      m_values.emplace_back(v, deduce_timepoint());
    }

    struct value_with_time {
      value_with_time(const Type& v,
                      rpp::schedulers::clock_type::time_point timepoint)
          : value{v}, timepoint{timepoint} {}
   ...
   private:
    rpp::schedulers::clock_type::time_point deduce_timepoint() {
      if (std::numeric_limits<rpp::schedulers::duration>::max() ==
          m_duration_limit)
        return rpp::schedulers::clock_type::time_point{};

      auto now = rpp::schedulers::clock_type::now();
      while (!m_values.empty() &&
             (now - m_values.front().timepoint > m_duration_limit))
        m_values.pop_front();
      return now;
    }
 ...
 public:
  ...
  replay_subject_base(size_t count, rpp::schedulers::duration duration)
      : m_state{disposable_wrapper_impl<replay_state>::make(
            std::max<size_t>(1, count), duration)} {}
...
/**
 * @brief Same as rpp::subjects::publish_subject but send all earlier emitted
 * values to any new observers.
 *
...
/**
 * @brief Same as rpp::subjects::replay_subject but
 * on_next/on_error/on_completed calls are serialized via mutex.
 * @details When you are using ordinary rpp::subjects::replay_subject, then you
 ...
 */
template <rpp::constraint::decayed_type Type>
class serialized_replay_subject final
    : public details::replay_subject_base<Type, true> {
 public:

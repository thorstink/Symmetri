 ...
 * of tasks should be manually dispatched
...
class run_loop final {
  ...
  class state_t final : public rpp::details::base_disposable {
   public:
    ...
    std::shared_ptr<details::schedulable_base> pop(bool wait) {
      while (!is_disposed()) {
        ...
        if (is_any_ready_schedulable_unsafe(now)) return m_queue.pop();
   ...
   private:
    bool is_any_ready_schedulable_unsafe(
        time_point now = worker_strategy::now()) const {
      return !m_queue.is_empty() && (m_queue.top()->is_disposed() ||
                                     m_queue.top()->get_timepoint() <= now);
    }
  ...
  class worker_strategy {
   public:
    worker_strategy(const std::weak_ptr<state_t>& state) : m_state{state} {}
    ...
    void defer_to(time_point tp, Fn&& fn, Handler&& handler,
      ...
      if (const auto shared = m_state.lock())
        shared->emplace_and_notify(tp, std::forward<Fn>(fn),
 ...
 public:
  bool is_empty() const { return m_state->is_empty(); }

  bool is_any_ready_schedulable() const {
    return m_state->is_any_ready_schedulable();
  }

  void dispatch_if_ready() const { dispatch_impl(false); }

  void dispatch() const { dispatch_impl(true); }

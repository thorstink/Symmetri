 ...
 * thread with priority to time_point and order.
 * @warning Caller thread is thread where `schedule` called.
 *
 * @details When this scheduler passed to some operators, then caller thread is
 * thread where scheduling of some action happens. In most cases it is where
 * `on_next` was called.
 *
 ...
 * To have any visible impact you need to use it at least **twice** during same
 * observable. For example, `rpp::source::just` source uses it as default
 * scheduler as well as `rpp::operators::merge` operator (which just "owns" it
 * during subscription).
 *
...
class current_thread {
 public:
  ...
  class worker_strategy {
   public:
    ...
    static void defer_for(duration duration, Fn&& fn, Handler&& handler,
                          Args&&... args) {
      if (handler.is_disposed()) return;
      ...
      if (!get_queue()) {
        ...
        const auto timepoint =
            details::immediate_scheduling_while_condition<worker_strategy>(
                duration, is_queue_is_empty{queue}, fn, handler, args...);
        if (!timepoint || handler.is_disposed()) return drain_queue();

        get_queue()->emplace(timepoint.value(), std::forward<Fn>(fn),
                             std::forward<Handler>(handler),
                             std::forward<Args>(args)...);
        return drain_queue();
      }

      get_queue()->emplace(now() + duration, std::forward<Fn>(fn),
                           std::forward<Handler>(handler),
                           std::forward<Args>(args)...);
    }
    ...
    template <rpp::schedulers::constraint::schedulable_handler Handler,
              typename... Args, constraint::schedulable_fn<Handler, Args...> Fn>
    static void defer_to(time_point tp, Fn&& fn, Handler&& handler,
                         Args&&... args) {
      if (handler.is_disposed()) return;

      if (get_queue()) {
        get_queue()->emplace(tp, std::forward<Fn>(fn),
                             std::forward<Handler>(handler),
                             std::forward<Args>(args)...);
      } else {
        defer_for(tp - now(), std::forward<Fn>(fn),
 ...
 private:
  class own_queue_guard {
   public:
    own_queue_guard() : m_clear_on_destruction{!get_queue()} {
      if (m_clear_on_destruction) get_queue() = &m_queue;
    }
    ~own_queue_guard() {
      if (m_clear_on_destruction) drain_queue();
    }

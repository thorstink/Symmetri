 ...
 * @warning Expected to use this scheduler as local variable to share same
 * threads between different operators or as static variable
 *
 * @par Examples
...
class thread_pool final {
  ...
  class worker_strategy {
   public:
    ...
    void defer_for(duration duration, Fn&& fn, Handler&& handler,
                   Args&&... args) const {
      m_original_worker.schedule(duration, std::forward<Fn>(fn),
    ...
    void defer_to(time_point tp, Fn&& fn, Handler&& handler,
                  Args&&... args) const {
      m_original_worker.schedule(tp, std::forward<Fn>(fn),
                                 std::forward<Handler>(handler),
                                 std::forward<Args>(args)...);
    }

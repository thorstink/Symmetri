...
/**
 * \brief scheduler which schedules execution of schedulables via queueing tasks
 * to another thread with priority to time_point and order \warning Creates new
 * thread for each "create_worker" call. Any scheduled task will be queued to
 * created thread for execution with respect to time_point and number of task
 * \ingroup schedulers
 */
  ...
  class worker_strategy {
   public:
    ...
    bool is_subscribed() const {
      if (const auto locked = m_state.lock()) return locked->is_subscribed();
      return false;
    }
    ...
    void defer_at(time_point time_point,
      ...
      defer_at(time_point,
               new_thread_schedulable{*this, time_point,
    ...
    class state : public std::enable_shared_from_this<state> {
     public:
      ...
      void defer_at(time_point time_point, new_thread_schedulable&& fn) {
        if (m_sub->is_subscribed()) m_queue.emplace(time_point, std::move(fn));
      }

      void init_thread(const rpp::composite_subscription& sub) {
        m_thread = std::thread{
            [state = shared_from_this()]() { state->data_thread(); }};
        const auto callback =
            rpp::callback_subscription{[state = weak_from_this()] {
              const auto locked = state.lock();
              if (!locked) return;

              locked->m_queue.unsubscribe();

              if (locked->m_thread.joinable() &&
                  locked->m_thread.get_id() != std::this_thread::get_id())

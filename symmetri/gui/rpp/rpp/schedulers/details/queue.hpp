...
class schedulable_base {
 public:
  ...
  class advanced_call {
   public:
    advanced_call(
        std::variant<delay_from_now, delay_from_this_timepoint, delay_to> data)
        : m_data{data} {}

    const std::variant<delay_from_now, delay_from_this_timepoint, delay_to>&
    get() const {
      return m_data;
    }

    auto visit(const auto& fn) const { return std::visit(fn, m_data); }

    bool can_run_immediately() const noexcept {
      return visit(rpp::utils::overloaded{
          [](const delay_to&) { return false; },
          [](const auto& d) {
  ...
  void update_next(std::shared_ptr<schedulable_base>&& next) {
    if (next) next->set_next(std::move(m_next));
    m_next = std::move(next);
  }
 ...
 protected:
  ...
  auto get_advanced_call_handler() const {
    return rpp::utils::overloaded{
        [](const delay_from_now& v) { return NowStrategy::now() + v.value; },
        [this](const delay_from_this_timepoint& v) {
          return get_timepoint() + v.value;
        },
        [](const delay_to& v) { return v.value; }};
  }
...
template <typename NowStrategy, rpp::constraint::decayed_type Fn,
...
class specific_schedulable final : public schedulable_base {
 public:
  ...
  explicit specific_schedulable(const time_point& time_point, TFn&& in_fn,
                                TArgs&&... in_args)
  ...
  void on_error(const std::exception_ptr& ep) const override {
    m_args.template get<0>().on_error(ep);
  }
...
class optional_mutex {
 public:
  ...
  optional_mutex(Mutex* mutex) : m_mutex{mutex} {}
...
class schedulables_queue {
 public:
  ...
  void emplace(const time_point& timepoint, Fn&& fn, Handler&& handler,
               Args&&... args) {
    using schedulable_type =
        specific_schedulable<NowStrategy, std::decay_t<Fn>,
                             std::decay_t<Handler>, std::decay_t<Args>...>;

    emplace_impl(std::make_shared<schedulable_type>(
        timepoint, std::forward<Fn>(fn), std::forward<Handler>(handler),
        std::forward<Args>(args)...));
  }

  void emplace(const time_point& timepoint,
               std::shared_ptr<schedulable_base>&& schedulable) {
  ...
  const std::shared_ptr<schedulable_base>& top() const { return m_head; }
 ...
 private:
  void emplace_impl(std::shared_ptr<schedulable_base>&& schedulable) {
    ...
    while (const auto& next = current->get_next()) {
      if (schedulable->get_timepoint() < next->get_timepoint()) break;
      current = next.get();
    }

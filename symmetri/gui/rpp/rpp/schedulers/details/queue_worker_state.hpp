...
class schedulable {
 public:
  schedulable(time_point time_point, size_t id, SchedulableFn&& fn)
      : m_time_point{time_point}, m_id{id}, m_function{std::move(fn)} {}
  ...
  bool operator<(const schedulable& other) const {
    return std::tie(m_time_point, m_id) >=
           std::tie(other.m_time_point, other.m_id);
  }
...
class queue_worker_state {
 public:
  ...
  void emplace(time_point time_point,
               constraint::inner_schedulable_fn auto&& fn) {
  ...
  bool pop_if_ready(std::optional<SchedulableFn>& out) {
    std::lock_guard lock{m_mutex};
    if (!is_any_ready_schedulable_unsafe()) return false;
  ...
  bool pop_with_wait(std::optional<SchedulableFn>& out) {
    while (m_subscription->is_subscribed()) {
      ...
      m_cv.wait(lock, [&] {
        return !m_subscription->is_subscribed() || !m_queue.empty();
      });

      if (m_queue.empty() ||
          !m_cv.wait_until(lock, m_queue.top().get_time_point(), [&] {
            return !m_subscription->is_subscribed() ||
                   is_any_ready_schedulable_unsafe();
          }))
        continue;

      if (!m_subscription->is_subscribed()) return false;
 ...
 private:
  void emplace_safe(time_point time_point,
                    constraint::inner_schedulable_fn auto&& fn) {
    std::lock_guard lock{m_mutex};
    if (m_subscription->is_subscribed())
      m_queue.emplace(time_point, ++m_current_id,
                      std::forward<decltype(fn)>(fn));
  }

  bool is_any_ready_schedulable_unsafe() const {
    return !m_queue.empty() &&
           m_queue.top().get_time_point() <= clock_type::now();
  }

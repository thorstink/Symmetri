...
class test_scheduler final {
 public:
  static inline rpp::schedulers::time_point s_current_time{
  ...
  struct state {
    ...
    template <rpp::schedulers::constraint::schedulable_handler Handler,
              typename... Args,
              rpp::schedulers::constraint::schedulable_fn<Handler, Args...> Fn>
    void schedule(rpp::schedulers::time_point time_point, Fn&& fn,
                  Handler&& handler, Args&&... args) {
      schedulings.push_back(time_point);
      queue.emplace(time_point, std::forward<Fn>(fn),
    ...
    void drain() {
      while (!queue.is_empty()) {
        if (queue.top()->get_timepoint() > s_current_time) return;

        auto fn = queue.top();
        queue.pop();

        if (fn->is_disposed()) continue;
        ...
        if (auto new_timepoint = (*fn)()) {
          ...
          schedulings.push_back(
              std::max(s_current_time, new_timepoint.value()));
  ...
  class worker_strategy {
   public:
    worker_strategy(std::weak_ptr<state> state) : m_state{std::move(state)} {}

    template <rpp::schedulers::constraint::schedulable_handler Handler,
              typename... Args,
              rpp::schedulers::constraint::schedulable_fn<Handler, Args...> Fn>
    void defer_for(rpp::schedulers::duration duration, Fn&& fn,
                   Handler&& handler, Args&&... args) const {
      if (auto locked = m_state.lock()) {
        locked->schedule(now() + duration, std::forward<Fn>(fn),
                         std::forward<Handler>(handler),
                         std::forward<Args>(args)...);
        locked->drain();
      }

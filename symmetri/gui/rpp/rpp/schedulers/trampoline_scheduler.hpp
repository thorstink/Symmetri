...
/**
 * \brief Schedules execution of schedulables via queueing tasks to the caller
 * thread with priority to time_point and order. \warning Caller thread is
 * thread where "schedule" called. \warning For example, in case of merging
 * values from two threads/observables and then using "trampoline", then it
 * would be thread_local for each emission/thread, not one thread selected for
 * all threads.
 *
 * \par Example
...
class trampoline final : public details::scheduler_tag {
  ...
  class worker_strategy {
   public:
    ...
    void defer_at(time_point time_point,
                  constraint::schedulable_fn auto&& fn) const {
      if (!m_sub.is_subscribed()) return;

      const bool someone_owns_queue = s_queue.has_value();

      const auto drain_on_exit =
          utils::finally_action(!someone_owns_queue ? &drain_queue : +[] {});

      if (!someone_owns_queue) {
        s_queue = std::priority_queue<current_thread_schedulable>{};

        if (!details::immediate_scheduling_while_condition(
                time_point, fn, m_sub, []() { return s_queue->empty(); }))
          return;

        // update time to make it more accurate due to we are going to push it
        // to queue
        time_point = std::max(now(), time_point);
      }

      defer_at(time_point,
               trampoline_schedulable{*this, time_point,
  ...
  static void drain_queue() {
    ...
    while (!s_queue->empty()) {
      ...
      // firstly we need to pop schedulable from queue due to execution of
      // function can add new schedulable
      s_queue->pop();
  ...
  static void wait_and_extract_executable_if_subscribed(
      const current_thread_schedulable& schedulable,
      std::optional<trampoline_schedulable>& out) {
    if (!schedulable.is_subscribed()) return;

    // wait only if needed!
    if (const auto requested_time = schedulable.get_time_point();
  ...
  class current_thread_schedulable
      : public details::schedulable<trampoline_schedulable> {
   public:
    current_thread_schedulable(time_point time_point, std::invocable auto&& fn,
                               rpp::composite_subscription subscription)
        : schedulable(time_point, get_thread_local_id(),
                      std::forward<decltype(fn)>(fn)),
  ...
  /**
   * \brief Optional thread_local schedulable queue. If optional has value ->
   * someone just owns thread.
   */
  inline static thread_local std::optional<
      std::priority_queue<current_thread_schedulable>>
      s_queue{};

 public:
  static utils::finally_action<void (*)()>
  own_queue_and_drain_finally_if_not_owned() {
    const bool someone_owns_queue = s_queue.has_value();
  ...
  static auto create_worker(
      const rpp::composite_subscription& sub = composite_subscription{}) {

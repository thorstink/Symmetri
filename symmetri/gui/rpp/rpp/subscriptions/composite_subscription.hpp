...
namespace rpp {
/**
 * \brief rpp::subscription_base with ability to add some dependent
 * subscriptions as a part of this one: in case of initiation of unsubscribe of
 * this subscription, then any dependent subscriptions will be unsubscribed too
 */
class composite_subscription final : public subscription_base {
 public:
  ...
  composite_subscription& operator=(const composite_subscription& other) =
      default;
  composite_subscription& operator=(composite_subscription&& other) noexcept =
      default;
  ...
  std::weak_ptr<details::subscription_state> add(
      const TSub& sub = TSub{}) const {
    if (static_cast<const subscription_base*>(&sub) ==
        static_cast<const subscription_base*>(this))
   ...
   */
  std::weak_ptr<details::subscription_state> add(
      const callback_subscription& sub) const {
  ...
  composite_subscription make_child() const {
    ...
    ret.add(
        [weak_handle = add(ret),
         state = std::weak_ptr{std::static_pointer_cast<state>(get_state())}] {
          // add cleanup
  ...
  bool is_empty() const { return !get_state(); }
  ...
  class state final : public details::subscription_state {
   public:
    ...
    void add(std::shared_ptr<details::subscription_state> sub) {
      if (!sub || !sub->is_subscribed()) return;
    ...
    void remove(const std::shared_ptr<details::subscription_state>& sub) {
      while (true) {
        ...
        if (expected == DepsState::Unsubscribed) return;
      }
    }
    ...
    void on_unsubscribe() override {
      while (true) {
        ...
        if (m_state.compare_exchange_strong(expected, DepsState::Unsubscribed,
                                            std::memory_order::acq_rel)) {
          rpp::utils::for_each(
              m_deps, std::mem_fn(&details::subscription_state::unsubscribe));
          m_deps.clear();
          return;
        }
      }
   ...
   private:
    enum class DepsState : uint8_t {
      None,  //< default state
      Edit,  //< set it during adding new element into deps or removing. After
             // success -> FallBack to None

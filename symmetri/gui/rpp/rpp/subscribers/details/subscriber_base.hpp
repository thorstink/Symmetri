...
/**
 * \brief base implementation of subscriber with possibility to obtain
 * observer's callbacks, query subscription state, unsubscribe and etc. Each
 * observer's callback checks for actual subscription \tparam Type type of
 * values expected by this subscriber
 */
class subscriber_base {
 public:
  subscriber_base(
      composite_subscription&& subscription = composite_subscription{})

...
// some objects can't be copy/moved-assigned, but can be copy/move constructed
// (like immutable lambdas), so, use trick with optional and emplace.
template <typename Callable>
class copy_assignable_callable {
 public:
  copy_assignable_callable(const Callable& callable) : m_callable{callable} {}
  ...
  copy_assignable_callable(copy_assignable_callable&& other) noexcept(
  ...
  copy_assignable_callable& operator=(
      copy_assignable_callable&& other) noexcept {
    if (this == &other) return *this;
    if (other.m_callable.has_value())
  ...
  decltype(auto) operator()(auto&&... args) {
    return (*m_callable)(std::forward<decltype(args)>(args)...);
  }
...
class finally_action {
 public:
  finally_action(Fn&& fn) : m_fn{std::move(fn)} {}

  finally_action(const Fn& fn) : m_fn{fn} {}

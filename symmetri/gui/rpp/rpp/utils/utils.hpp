...
using iterable_value_t =
    std::iter_value_t<decltype(std::begin(std::declval<T>()))>;

namespace details {
template <template <typename...> typename Base>
struct traits {
  template <typename... Types>
  constexpr static rpp::utils::tuple<Types...> extract_params(
      const Base<Types...>*);
...
concept is_base_of_v =
    !std::is_same_v<decltype(details::traits<Base>::extract_params(
                        std::declval<std::decay_t<T>*>())),
                    std::false_type>;

template <typename T, template <typename...> typename Base>
  requires is_base_of_v<T, Base>
using extract_base_type_params_t =
    decltype(details::traits<Base>::extract_params(
        std::declval<std::decay_t<T>*>()));
...
class value_with_mutex {
 public:
  ...
  explicit value_with_mutex(const T& v) : m_value{v} {}

  explicit value_with_mutex(T&& v) : m_value{std::move(v)} {}
...
struct unique_variant_t<std::variant<Ts...>, U, Us...>
    : unique_variant_t<std::variant<Ts...>, Us...> {};

template <typename... Ts, typename U, typename... Us>
struct unique_variant_t<std::variant<Ts...>, U, Us...>
    : public unique_variant_t<std::variant<Ts..., U>, Us...> {};
}  // namespace details

template <typename... Ts>
using unique_variant =
    typename details::unique_variant_t<std::variant<>, Ts...>::
        type;  // inspired by https://stackoverflow.com/a/57528226/17771792

#define RPP_CALL_DURING_CONSTRUCTION(...)            \
  RPP_NO_UNIQUE_ADDRESS rpp::utils::none _ = [&]() { \
    __VA_ARGS__;                                     \

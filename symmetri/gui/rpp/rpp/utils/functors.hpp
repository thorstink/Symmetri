...
struct rethrow_error_t {
  [[noreturn]] void operator()(const std::exception_ptr& err) const {
...
struct forwarding_on_next {
  void operator()(auto&& v, const auto& sub, const auto&...) const {
    sub.on_next(std::forward<decltype(v)>(v));
  }
};

struct forwarding_on_error {
  void operator()(const std::exception_ptr& err, const auto& sub,
                  const auto&...) const {
    sub.on_error(err);
  }
};
...
struct forwarding_on_next_for_pointer {
  void operator()(auto&& v, const auto& sub) const {
    sub->on_next(std::forward<decltype(v)>(v));
  }
};

struct forwarding_on_error_for_pointer {
  void operator()(const std::exception_ptr& err, const auto& sub) const {
    sub->on_error(err);
  }
};

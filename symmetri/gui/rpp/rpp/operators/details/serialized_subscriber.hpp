...
auto make_serialized_subscriber(
    TSub&& subscriber,
    const std::shared_ptr<TSerializationPrimitive>& primitive) {
  auto sub = subscriber.get_subscription();
  return create_subscriber_with_state<
      utils::extract_subscriber_type_t<std::decay_t<TSub>>>(
      std::move(sub), forwarding_on_next_under_lock{},
      forwarding_on_error_under_lock{}, forwarding_on_completed_under_lock{},
      std::forward<TSub>(subscriber), primitive);
}

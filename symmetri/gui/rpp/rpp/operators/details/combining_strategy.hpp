...
class combining_state
    : public rpp::details::enable_wrapper_from_this<combining_state<Observer>>,
...
template <template <typename...> typename TState,
          template <auto, typename...> typename TStrategy, typename TSelector,
...
struct combining_operator_t {
  ...
  using updated_disposable_strategy = ::rpp::details::observables::
      default_disposable_strategy_selector;  // TODO: sum of Prev + TObservables
 ...
 private:
  ...
  static auto subscribe_impl(Observer&& observer, const TSelector& selector,
    ...
    using State =
        TState<Observer, TSelector, Type,
               rpp::utils::extract_observable_type_t<TObservables>...>;

    const auto d = rpp::disposable_wrapper_impl<State>::make(
        std::forward<Observer>(observer), selector);
    auto state = d.lock();
    state->get_observer_under_lock()->set_upstream(d.as_weak());

    subscribe<std::decay_t<Type>>(
        state, std::index_sequence_for<TObservables...>{}, observables...);

    return rpp::observer<
        Type,
        TStrategy<0, std::decay_t<Observer>, TSelector, Type,
                  rpp::utils::extract_observable_type_t<TObservables>...>>{
        std::move(state)};
  }

  template <typename ExpectedValue, rpp::constraint::observer Observer,
            size_t... I>
  static void subscribe(
      const std::shared_ptr<TState<
          Observer, TSelector, ExpectedValue,
    ...
    (...,
     observables.subscribe(
         rpp::observer<
             rpp::utils::extract_observable_type_t<TObservables>,
             TStrategy<I + 1, Observer, TSelector, ExpectedValue,
                       rpp::utils::extract_observable_type_t<TObservables>...>>{
             state}));
  }
};

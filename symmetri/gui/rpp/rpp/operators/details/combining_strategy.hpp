...
template <template <typename...> typename TDisposable,
...
struct combining_operator_t {
 ...
 private:
  ...
  static auto subscribe_impl(Observer&& observer, const TSelector& selector,
    ...
    using Disposable =
        TDisposable<Observer, TSelector, Type,
                    rpp::utils::extract_observable_type_t<TObservables>...>;
    ...
    subscribe<std::decay_t<Type>>(
        locked, std::index_sequence_for<TObservables...>{}, observables...);
  ...
  static void subscribe(
      const std::shared_ptr<TDisposable<

...
class forwarding_subject {
 ...
 public:
  using optimal_disposables_strategy =
      typename subjects::details::subject_state<
          Type, false>::optimal_disposables_strategy::template add<1>;

  explicit forwarding_subject(
      disposable_wrapper_impl<rpp::refcount_disposable> refcount)
  ...
  auto get_observable() const {
    return subjects::details::create_subject_on_subscribe_observable<
        Type, optimal_disposables_strategy>(
        [state = m_state.as_weak(),
         refcount = m_refcount]<rpp::constraint::observer_of_type<Type> TObs>(
            TObs&& observer) {
 ...
 private:
  ...
  disposable_wrapper_impl<subjects::details::subject_state<Type, false>>
      m_state = disposable_wrapper_impl<
          subjects::details::subject_state<Type, false>>::make();
};
}  // namespace rpp::operators::details

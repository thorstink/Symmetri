...
/**
 * Functor (type-erasure) of throttler (trigger observable) for on_next
 * operator.
 */
struct take_until_throttler_on_next {
  void operator()(auto&&, const auto& subscriber,
...
struct take_until_state_with_serialized_spinlock : take_until_state {
  ...
  // to get error from "until observable" immediately
...
struct take_until_impl {
  ...
  auto operator()(TSub&& in_subscriber) const {
    auto state = std::make_shared<take_until_state_with_serialized_spinlock>(
    ...
    m_until_observable.subscribe(create_subscriber_with_state<TriggerType>(
        state->children_subscriptions.make_child(),
        take_until_throttler_on_next{}, take_until_throttler_on_error{},
        take_until_throttler_on_completed{}, subscriber, state));

    auto subscription = state->children_subscriptions.make_child();
    return create_subscriber_with_state<Type>(
        std::move(subscription), take_until_on_next{}, take_until_on_error{},
        take_until_on_completed{}, std::move(subscriber), std::move(state));
  }
};

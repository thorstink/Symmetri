...
template <rpp::constraint::observer TObserver>
struct take_until_throttle_observer_strategy
...
struct take_until_t {
  ...
  auto lift(Observer&& observer) const {
    ...
    return rpp::observer<Type,
                         take_until_observer_strategy<std::decay_t<Observer>>>(
        std::move(ptr));
  }
};
}  // namespace rpp::operators::details
 ...
 * @warning The take_until subscribes and begins mirroring the source
 ...
 second Observable emits an item or sends a on_error/on_completed notification,
 the Observable returned by take_until stops mirroring the source Observable and
 terminates.
 *
 * @marble take_until
 {
...
auto take_until(TObservable&& until_observable) {
  return details::take_until_t<std::decay_t<TObservable>>{

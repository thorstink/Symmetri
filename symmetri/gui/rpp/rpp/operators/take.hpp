...
struct take_on_next {
  void operator()(auto&& value, const constraint::subscriber auto& subscriber,
                  const take_state& state) const {
    if (state.count > 0) {
      --state.count;
      subscriber.on_next(std::forward<decltype(value)>(value));
    }

    if (state.count == 0) subscriber.on_completed();
  }
};
...
struct take_impl {
  ...
  auto operator()(TSub&& subscriber) const {
    ...
    return create_subscriber_with_dynamic_state<Type>(
        std::move(subscription), take_on_next{}, utils::forwarding_on_error{},

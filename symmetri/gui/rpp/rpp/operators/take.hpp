...
struct take_observer_strategy {
  ...
  void on_next(T&& v) const {
    ...
    if (count == 0) observer.on_completed();
  }
...
/**
 * @brief Emit only first `count` items provided by observable, then send
 `on_completed`
 *
 * @marble take
 {
 ...
 * @details Actually this operator just emits emissions while counter is not
 zero and decrements counter on each emission
 *
 ...
 */
inline auto take(size_t count) { return details::take_t{count}; }
}  // namespace rpp::operators

...
template <rpp::constraint::observer TObserver,
          rpp::schedulers::constraint::scheduler Scheduler>
struct throttle_observer_strategy {
  ...
  void on_next(T&& v) const {
    ...
    if (!last_emission_time_point ||
        now >= last_emission_time_point.value() + duration) {
...
struct throttle_t
    : lift_operator<throttle_t<Scheduler>, rpp::schedulers::duration> {
  using lift_operator<throttle_t<Scheduler>,
                      rpp::schedulers::duration>::lift_operator;
...
/**
* @brief Emit emission from an Observable and then ignore subsequent values
...
*
* @details Actually this operator just keeps time of last emission, skips values
​​until duration has passed, then forwards value, updates timepoint and
etc...
*
* @par Performance notes:
* - No any heap allocations at all
* - No any copies/moves of emissions, just passing by const& to predicate and
then forwarding
* - Obtaining "now" every emission
*
* @param period is period of time to skip subsequent emissions
* @tparam Scheduler is type used to determine `now()`. Shouldn't be used in
production code
*
...
*/
template <rpp::schedulers::constraint::scheduler
              Scheduler /* = rpp::schedulers::immediate*/>

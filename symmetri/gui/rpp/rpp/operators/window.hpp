...
class window_observer_strategy {
 ...
 public:
  ...
  void on_next(T&& v) const {
    // need to send new subject due to NEW item appeared (we avoid sending new
  ...
  void on_error(const std::exception_ptr& err) const {
    if (m_subject_data) m_subject_data->observer.on_error(err);
    m_observer.on_error(err);
  }

  void on_completed() const {
    if (m_subject_data) m_subject_data->observer.on_completed();
 ...
 private:
  std::shared_ptr<refcount_disposable> m_disposable =
      disposable_wrapper_impl<refcount_disposable>::make().lock();
 ...
 */
inline auto window(size_t count) { return details::window_t{count}; }
}  // namespace rpp::operators

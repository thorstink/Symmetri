...
struct window_on_next {
  ...
  void operator()(auto&& value, const auto& subscriber,
                  const window_state<Type>& state) const {
    ...
    state.subject.get_subscriber().on_next(
...
struct window_lift_impl {
  ...
  auto operator()(TSub&& subscriber) const {
    ...
    return create_subscriber_with_dynamic_state<Type>(
        std::move(subscription), window_on_next{}, window_on_error{},
        window_on_completed{}, std::forward<TSub>(subscriber),
...
auto window_impl(TObs&& obs, size_t window_size) {
  return std::forward<TObs>(obs).template lift<windowed_observable<Type>>(

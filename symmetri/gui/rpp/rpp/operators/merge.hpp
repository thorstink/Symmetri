...
struct merge_on_completed {
  void operator()(const constraint::subscriber auto& sub,
    ...
    if (state->count_of_on_completed_needed.fetch_sub(
...
struct merge_on_next {
  ...
  void operator()(const TObs& new_observable,
    ...
    new_observable.subscribe(create_subscriber_with_state<ValueType>(
        state->children_subscriptions.make_child(), merge_forwarding_on_next{},
        merge_on_error{}, merge_on_completed{}, sub, state));
  }
};
...
struct merge_impl {
  ...
  auto operator()(TSub&& in_subscriber) const {
    auto state = std::make_shared<merge_state_with_serialized_mutex>(
    ...
    auto subscriber = make_serialized_subscriber(
        ...
        std::shared_ptr<std::mutex>{state, &state->mutex});
    ...
    return create_subscriber_with_state<Type>(
        std::move(subscription), merge_on_next{}, merge_on_error{},
        merge_on_completed{}, std::move(subscriber), std::move(state));
  }
};

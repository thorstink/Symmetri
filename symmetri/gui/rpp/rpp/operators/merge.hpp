...
class merge_state final {
 public:
  ...
  void increment_on_completed() {
    m_on_completed_needed.fetch_add(1, std::memory_order::seq_cst);
  }
  ...
  rpp::utils::pointer_under_lock<TObserver> get_observer_under_lock() {
    return m_observer;
  }
...
template <rpp::constraint::observer TObserver>
struct merge_observer_inner_strategy final
...
template <rpp::constraint::observer TObserver>
class merge_observer_strategy final
    : public merge_observer_base_strategy<TObserver> {
 public:
...
struct merge_with_t {
  ...
  struct operator_traits {
    static_assert(
        (std::same_as<T, rpp::utils::extract_observable_type_t<TObservables>> &&
         ...),
  ...
  void subscribe(Observer&& observer,
    ...
    // Need to take ownership over current_thread in case of inner-observables
    // also using it
    auto drain_on_exit = rpp::schedulers::current_thread::
 ...
 private:
  ...
  static void apply(const merge_observer_strategy<Observer>& strategy,
 ...
 *
 * @details Actually it subscribes on each observable from emissions. Resulting
 observables completes when ALL observables completes
 *
 ...
 */
inline auto merge() { return details::merge_t{}; }
 ...
 *
 * @details Actually it subscribes on each observable. Resulting observables
 completes when ALL observables completes
 *
 * @par Performance notes:
 * - 2 heap allocation (1 for state, 1 to convert observer to dynamic_observer)
 * - Acquiring mutex during all observer's calls
...
auto merge_with(TObservable&& observable, TObservables&&... observables) {
  return details::merge_with_t<std::decay_t<TObservable>,
                               std::decay_t<TObservables>...>{

...
template <rpp::constraint::decayed_type Type,
          rpp::constraint::observer_strategy<Type> ObserverStrategy>
...
class subscribe_t<ObserverStrategy> {
 public:
  ...
  void operator()(const Observable& observable) const& {
    static_assert(rpp::constraint::observer_strategy<
                      ObserverStrategy,
                      rpp::utils::extract_observable_type_t<Observable>>,
                  "observer and observable should be of same type");
...
template <rpp::constraint::decayed_type Type,
          rpp::constraint::observer_strategy<Type> ObserverStrategy>
class subscribe_t<rpp::composite_disposable_wrapper,
                  observer<Type, ObserverStrategy>> {
 public:
  explicit subscribe_t(rpp::composite_disposable_wrapper&& d,
                       observer<Type, ObserverStrategy>&& observer)
      : m_disposable{std::move(d)}, m_observer{std::move(observer)} {}
...
class subscribe_t<rpp::composite_disposable_wrapper, ObserverStrategy> {
 public:
  ...
  explicit subscribe_t(rpp::composite_disposable_wrapper&& d,
      ...
      : m_disposable{std::move(d)}, m_observer_strategy{observer_strategy} {}
...
class subscribe_t<OnNext, OnError, OnCompleted> {
 public:
  template <rpp::constraint::decayed_same_as<OnNext> TOnNext,
            rpp::constraint::decayed_same_as<OnError> TOnError,
            rpp::constraint::decayed_same_as<OnCompleted> TOnCompleted>
    requires(!constraint::decayed_same_as<
                TOnNext, subscribe_t<OnNext, OnError, OnCompleted>>)
  explicit subscribe_t(TOnNext&& on_next, TOnError&& on_error,
                       TOnCompleted&& on_completed)
  ...
  void operator()(const rpp::observable<Type, Strategy>& observable) && {
    static_assert(std::invocable<OnNext, Type>,
                  "OnNext should be suitable for type of observable");
    observable.subscribe(std::move(m_on_next), std::move(m_on_error),
                         std::move(m_on_completed));
  }
...
template <typename OnNext, std::invocable<const std::exception_ptr&> OnError,
          std::invocable<> OnCompleted>
class subscribe_t<rpp::composite_disposable_wrapper, OnNext, OnError,
                  OnCompleted> {
 public:
  template <rpp::constraint::decayed_same_as<OnNext> TOnNext,
            rpp::constraint::decayed_same_as<OnError> TOnError,
            rpp::constraint::decayed_same_as<OnCompleted> TOnCompleted>
  explicit subscribe_t(rpp::composite_disposable_wrapper d, TOnNext&& on_next,
                       TOnError&& on_error, TOnCompleted&& on_completed)
      : m_disposable{std::move(d)},
        m_on_next{std::forward<TOnNext>(on_next)},
        m_on_error{std::forward<TOnError>(on_error)},
        m_on_completed{std::forward<TOnCompleted>(on_completed)} {}
  ...
  rpp::composite_disposable_wrapper operator()(
    ...
    static_assert(std::invocable<OnNext, Type>,
                  "OnNext should be suitable for type of observable");
    observable.subscribe(m_disposable, std::move(m_on_next),
                         std::move(m_on_error), std::move(m_on_completed));
  ...
  template <rpp::constraint::decayed_type Type,
            rpp::constraint::observable_strategy<Type> Strategy>
  rpp::composite_disposable_wrapper operator()(
      const rpp::observable<Type, Strategy>& observable) const& {
    static_assert(std::invocable<OnNext, Type>,
                  "OnNext should be suitable for type of observable");
...
concept on_next_like =
    (!rpp::utils::is_not_template_callable<OnNext> ||
     std::invocable<OnNext, rpp::utils::convertible_to_any>) &&
    (!rpp::constraint::decayed_same_as<OnNext,
 ...
 *
 * @warning Observer must be moved in to subscribe method. (Not recommended) If
 * you need to copy observer, convert it to dynamic_observer
 *
 ...
 *
 * @param disposable is disposable to be attached to observer. If disposable is
 * nullptr or disposed -> no any subscription happens
 *
 * @ingroup utility_operators
 */
template <rpp::constraint::decayed_type Type,
          rpp::constraint::observer_strategy<Type> ObserverStrategy>
auto subscribe(rpp::composite_disposable_wrapper disposable,
               observer<Type, ObserverStrategy>&& observer) {
...
/**
 * @brief  Subscribes passed observer strategy to emissions from this observable
 * via construction of observer
 *
 * @ingroup utility_operators
 */
template <rpp::constraint::observer_strategy_base ObserverStrategy>
...
auto subscribe(rpp::composite_disposable_wrapper disposable,
               ObserverStrategy&& observer_strategy) {
  return details::subscribe_t{
      std::move(disposable), std::forward<ObserverStrategy>(observer_strategy)};
}

/**
 * @brief Construct rpp::lambda_observer on the fly and subscribe it to
 * emissions from observable
 *
 * @ingroup utility_operators
 */
template <details::on_next_like OnNext = rpp::utils::empty_function_any_t,
...
auto subscribe(OnNext&& on_next = {}, OnError&& on_error = {},
               OnCompleted&& on_completed = {}) {
  return details::subscribe_t{std::forward<OnNext>(on_next),
...
template <details::on_next_like OnNext = rpp::utils::empty_function_any_t,
          std::invocable<const std::exception_ptr&> OnError =
              rpp::utils::rethrow_error_t,
          std::invocable<> OnCompleted = rpp::utils::empty_function_t<>>
auto subscribe(rpp::composite_disposable_wrapper d, OnNext&& on_next = {},
               OnError&& on_error = {}, OnCompleted&& on_completed = {}) {
  return details::subscribe_t{std::move(d), std::forward<OnNext>(on_next),
                              std::forward<OnError>(on_error),
                              std::forward<OnCompleted>(on_completed)};
}
...
template <details::on_next_like OnNext = rpp::utils::empty_function_any_t,
          std::invocable<const std::exception_ptr&> OnError =
              rpp::utils::rethrow_error_t,
          std::invocable<> OnCompleted = rpp::utils::empty_function_t<>>
auto subscribe_with_disposable(OnNext&& on_next = {}, OnError&& on_error = {},
                               OnCompleted&& on_completed = {}) {
  return subscribe(
      composite_disposable_wrapper::make(), std::forward<OnNext>(on_next),
      std::forward<OnError>(on_error), std::forward<OnCompleted>(on_completed));
}
...
auto subscribe_with_disposable(OnNext&& on_next, OnCompleted&& on_completed) {
  return subscribe(composite_disposable_wrapper::make(),
                   std::forward<OnNext>(on_next), rpp::utils::rethrow_error_t{},
                   std::forward<OnCompleted>(on_completed));
}
}  // namespace rpp::operators

...
class group_by_state_base
    : public std::enable_shared_from_this<group_by_state_base> {
 public:
...
template <constraint::decayed_type TKey, constraint::decayed_type Type,
...
struct group_by_state final : group_by_state_base {
  group_by_state(const KeyComparator& comparator)
      : group_by_state_base{}, key_to_subject{comparator} {}
...
using grouped_observable_group_by =
    grouped_observable<TKey, ResValue,
                       details::group_by_on_subscribe<ResValue>>;
}
...
struct group_by_lift_impl {
  ...
  using StateType =
      group_by_state<TKey, utils::decayed_invoke_result_t<ValueSelector, Type>,
                     KeyComparator>;
 ...
 private:
  ...
  struct on_next {
    void operator()(auto&& val, const auto& subscriber,
                    const observer_state& state) const {
      auto key = state.key_selector(utils::as_const(val));
      auto [itr, inserted] = state.state->key_to_subject.try_emplace(key);

      if (inserted)
        subscriber.on_next(grouped_observable_group_by<TKey, ValueType>{
            key, group_by_on_subscribe<ValueType>{itr->second, state.state}});

      const auto& subject_sub = itr->second.get_subscriber();
      if (subject_sub.is_subscribed())
        subject_sub.on_next(
            state.value_selector(std::forward<decltype(val)>(val)));
    }
  };
  ...
  struct on_error {
    void operator()(const std::exception_ptr& err, const auto& subscriber,
                    const observer_state& state) const {
      state.state->broadcast([&err](const auto& sub) { sub.on_error(err); },
                             subscriber);
    }
  };
  ...
  struct on_completed {
    void operator()(const auto& subscriber, const observer_state& state) const {
      state.state->broadcast([](const auto& sub) { sub.on_completed(); },
                             subscriber);
    }
  };
 ...
 public:
  ...
  auto operator()(TSub&& subscriber) const {
    ...
    return create_subscriber_with_state<Type>(
        state->get_source_lifetime(), on_next{}, on_error{}, on_completed{},
...
auto group_by_impl(auto&& observable, KeySelector&& key_selector,
  ...
  using Res = grouped_observable_group_by<
      TKey, utils::decayed_invoke_result_t<ValueSelector, Type>>;
  using Lifter = group_by_lift_impl<Type, TKey, std::decay_t<KeySelector>,

...
template <
    ...
    rpp::constraint::observable TFallbackObservable,
...
class timeout_disposable final
 ...
 public:
  ...
  timeout_disposable(TObserver&& observer, rpp::schedulers::duration period,
                     const TFallbackObservable& fallback,
                     rpp::schedulers::time_point timeout)
      : m_observer_with_timeout{observer_with_timeout{std::move(observer),
                                                      timeout}},
        m_period{period},
        m_fallback{fallback} {}
  rpp::utils::pointer_under_lock<observer_with_timeout>
  get_observer_with_timeout_under_lock() {
...
struct timeout_disposable_wrapper {
  std::shared_ptr<timeout_disposable<TObserver, TFallbackObservable, Container>>
      disposable;
...
template <
    ...
    rpp::constraint::observable TFallbackObservable,
    rpp::details::disposables::constraint::disposables_container Container,
    rpp::schedulers::constraint::scheduler TScheduler>
struct timeout_observer_strategy {
  ...
  void on_next(T&& v) const {
    ...
    obs_with_timeout->timeout =
        rpp::schedulers::utils::get_worker_t<TScheduler>::now() +
        disposable->get_period();
  }

  void on_error(const std::exception_ptr& err) const noexcept {
    const auto obs_with_timeout =
        disposable->get_observer_with_timeout_under_lock();
    if (!disposable->is_disposed()) obs_with_timeout->observer.on_error(err);
  }

  void on_completed() const noexcept {
    const auto obs_with_timeout =
        disposable->get_observer_with_timeout_under_lock();
    if (!disposable->is_disposed()) obs_with_timeout->observer.on_completed();
  }
};

template <rpp::constraint::observable TFallbackObservable,
          rpp::schedulers::constraint::scheduler TScheduler>
struct timeout_t {
  template <rpp::constraint::decayed_type T>
  struct operator_traits {
    static_assert(rpp::constraint::observable_of_type<TFallbackObservable, T>,
                  "TFallbackObservable should be the same type as T");
  ...
  auto lift_with_disposables_strategy(Observer&& observer) const {
    ...
    const auto disposable = disposable_wrapper_impl<
        timeout_disposable<std::decay_t<Observer>, TFallbackObservable,
                           container>>::make(std::forward<Observer>(observer),
                                             period, fallback, timeout);
    auto ptr = disposable.lock();
    ptr->get_observer_with_timeout_under_lock()->observer.set_upstream(
        disposable.as_weak());

    const auto worker = scheduler.create_worker();
    using wrapper = timeout_disposable_wrapper<std::decay_t<Observer>,
                                               TFallbackObservable, container>;
    worker.schedule(
        timeout,
        [](wrapper& handler) -> rpp::schedulers::optional_delay_to {
          auto locked_obs_with_timeout =
              handler.disposable->get_observer_with_timeout_under_lock();
          if (rpp::schedulers::utils::get_worker_t<TScheduler>::now() <
              locked_obs_with_timeout->timeout)
          ...
          if (!handler.disposable->is_disposed()) {
            ...
            handler.disposable->get_fallback().subscribe(
                std::move(locked_obs_with_timeout->observer));
          }
          return std::nullopt;
        },
        wrapper{ptr});

    return rpp::observer<Type, timeout_observer_strategy<
                                   std::decay_t<Observer>, TFallbackObservable,
                                   container, TScheduler>>{std::move(ptr)};
  }
};
...
struct timeout_with_error_t {
  ...
  template <rpp::details::observables::constraint::disposables_strategy Prev>
  using updated_optimal_disposables_strategy =
      typename timeout_t<rpp::error_observable<int>, TScheduler>::
          template updated_optimal_disposables_strategy<Prev>;
  ...
  template <rpp::constraint::decayed_type Type,
            ...
            rpp::constraint::observer Observer>
  auto lift_with_disposables_strategy(Observer&& observer) const {
    return timeout_t<rpp::error_observable<Type>, TScheduler>{
        period,
        rpp::source::error<rpp::utils::extract_observer_type_t<Observer>>(
            std::make_exception_ptr(
                rpp::utils::timeout_reached{"Timeout reached"})),
        scheduler}
        .template lift_with_disposables_strategy<Type, DisposableStrategy>(
...
/**
 * @brief Forwards emissions from original observable, but subscribes on
 fallback observable if no any events during specified period of time (since
 last emission)
 *
 ...
 *
 * @param period is maximum duration between emitted items before a timeout
 occurs
...
template <rpp::constraint::observable TFallbackObservable,
          rpp::schedulers::constraint::scheduler TScheduler>
auto timeout(rpp::schedulers::duration period,
             TFallbackObservable&& fallback_observable,
             const TScheduler& scheduler) {
  return details::timeout_t<std::decay_t<TFallbackObservable>, TScheduler>{
      period, std::forward<TFallbackObservable>(fallback_observable),
      scheduler};
}

/**
 * @brief Forwards emissions from original observable, but emit error if no any
events during specified period of time (since last emission)
 *

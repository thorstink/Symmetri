...
/**
 * @brief Sends items from observable while items are satisfy predicate. When
 condition becomes false -> sends `on_completed`
 *
 * @marble take_while
 {
 ...
 * - No any copies/moves of emissions, just passing to predicate by const& and
 then forwarding
 *
 * @param predicate is predicate used to check items. Accepts value from
 observable and returns `true` if value should be forwarded and `false` if
 emissions should be stopped and observable should be terminated.

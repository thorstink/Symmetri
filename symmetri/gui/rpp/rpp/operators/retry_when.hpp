...
struct retry_when_state final {
  retry_when_state(TObserver&& in_observer, const TObservable& observable,
...
template <rpp::constraint::observer TObserver, typename TObservable,
          typename TNotifier>
void drain(
    const std::shared_ptr<retry_when_state<TObserver, TObservable, TNotifier>>&
        state);
  ...
  void on_error(const std::exception_ptr& err) const {
    try {
      state->notifier(err).subscribe(
          retry_when_impl_inner_strategy<TObserver, TObservable, TNotifier>{
              state});
    } catch (...) {
  ...
  while (!state->disposable.is_disposed()) {
    ...
    try {
      ...
      state->observable.subscribe(
          rpp::observer<value_type, retry_when_impl_strategy<
                                        TObserver, TObservable, TNotifier>>{
              state});
...
struct retry_when_t {
  ...
  void subscribe(TObserver&& observer, TObservable&& observable) const {
    const auto ptr = std::make_shared<
        retry_when_state<std::decay_t<TObserver>, std::decay_t<TObservable>,
                         std::decay_t<TNotifier>>>(

...
template <typename KeySelector, typename ValueSelector = std::identity,
...
template <typename InitialValue, typename Fn>
  requires(!utils::is_not_template_callable<Fn> ||
           std::same_as<std::decay_t<InitialValue>,
                        std::invoke_result_t<Fn, std::decay_t<InitialValue> &&,
                                             rpp::utils::convertible_to_any>>)
...
auto tap(OnNext&& on_next, OnCompleted&& on_completed);

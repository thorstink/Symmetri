...
struct member_overload<Type, SpecificObservable, switch_map_tag> {
  /**
  * \brief convert an Observable that emits Observables into a single Observable
  ...
  * \return new specific_observable with the switch_map operator as most recent
  operator.
  ...
  */
  ...
  auto switch_map(Callable&& callable) const&
    requires is_header_included<switch_map_tag, Callable>
  {
    return switch_map_impl<Type>(*static_cast<const SpecificObservable*>(this),
                                 std::forward<Callable>(callable));
  }

  template <switch_map_callable<Type> Callable>
      auto switch_map(Callable&& callable) &&
        requires is_header_included<switch_map_tag, Callable>
  {
    return switch_map_impl<Type>(

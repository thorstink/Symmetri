...
struct member_overload<Type, SpecificObservable, map_tag> {
  /**
   * \brief Transform the items emitted by an Observable via applying a function
   to each item and emitting result
   * \note The Map operator can keep same type of value or change it to some
   another type.
   *
   * \marble map
   {
   ...
   * \return new specific_observable with the Map operator as most recent
   operator.
  ...
  auto map(Callable&& callable) const&
    requires is_header_included<map_tag, Callable>
  {
    return static_cast<const SpecificObservable*>(this)
        ->template lift<utils::decayed_invoke_result_t<Callable, Type>>(
            map_impl<Type, std::decay_t<Callable>>{

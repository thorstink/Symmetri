...
concept flat_map_callable =
    std::invocable<Fn, Type> &&
    constraint::observable<utils::decayed_invoke_result_t<Fn, Type>>;
...
struct member_overload<Type, SpecificObservable, flat_map_tag> {
  /**
  * \brief Transform emissions to observables via provided function and then
  merge emissions from such an observables
  *
  ...
  * \return new specific_observable with the flat_map operator as most recent
  operator.
  ...
  */
  ...
  auto flat_map(Callable&& callable) const&
    requires is_header_included<flat_map_tag, Callable>
  {
    return flat_map_impl<Type>(*static_cast<const SpecificObservable*>(this),
                               std::forward<Callable>(callable));
  }

  template <flat_map_callable<Type> Callable>
      auto flat_map(Callable&& callable) &&
        requires is_header_included<flat_map_tag, Callable>
  {
    return flat_map_impl<Type>(

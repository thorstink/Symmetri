...
struct member_overload<Type, SpecificObservable, skip_tag> {
   ...
   *
   * \details Actually this operator just decrements counter and starts to
   forward emissions when counter reaches zero.
   *
   * \param count amount of items to be skipped
   * \return new specific_observable with the skip operator as most recent
   operator.
        ...
        requires is_header_included<skip_tag, Args...>
  {
    return std::move(*static_cast<SpecificObservable*>(this))
        .template lift<Type>(skip_impl<Type>{count});
  }
};
}  // namespace rpp::details

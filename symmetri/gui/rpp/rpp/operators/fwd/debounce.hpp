...
struct member_overload<Type, SpecificObservable, debounce_tag> {
  /**
   * \brief Only emit emission if specified period of time has passed without
   any other emission. On each new emission timer reset.
   *
   * \marble debounce
   {
   ...
   * \return new specific_observable with the debounce operator as most recent
   operator.
   ...
   *    - Saves emission
   *    - Schedule action to send this emission with check if no any new
   emissions
   * - <b>OnError</b>
  ...
  auto debounce(schedulers::duration period,
    ...
    requires is_header_included<debounce_tag, TScheduler>
  {
    return static_cast<const SpecificObservable*>(this)->template lift<Type>(
        debounce_impl<Type, TScheduler>{period, scheduler});
  }

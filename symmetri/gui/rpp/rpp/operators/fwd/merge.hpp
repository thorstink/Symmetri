...
struct member_overload<Type, SpecificObservable, merge_tag> {
  ...
  * \warning According to observable contract
  ...
  observable should be serialized, so, resulting observable uses mutex to
  ...
  *
  * \return new specific_observable with the merge operator as most recent
  operator.
  ...
  * - <b>OnCompleted</b>
  *    - Just forwards original on_completed when all observables emit
  on_completed
  *
  ...
  */
  ...
  auto merge() const&
    requires(is_header_included<merge_tag, Args...> &&
  ...
  template <typename... Args>
      ...
      *
      * \details Actually it subscribes on each observable. Resulting
      observables completes when ALL observables completes
      *

...
struct member_overload<Type, SpecificObservable, scan_tag> {
   ...
   *
   * \details Acttually this operator applies provided accumulator function to
   seed and new emission, emits resulting value and updates seed value for next
   emission
   *
   * \param initial_value initial value for seed which will be applied for first
   value from observable (instead of emitting this as first value). Then it will
   be replaced with result and etc.
   ...
   *
   * \return new specific_observable with the scan operator as most recent
   operator.
  ...
  auto scan(Result&& initial_value, AccumulatorFn&& accumulator) const&
    requires is_header_included<scan_tag, Result, AccumulatorFn>
  {
    return static_cast<const SpecificObservable*>(this)
        ->template lift<std::decay_t<Result>>(
            scan_impl<Type, std::decay_t<Result>, std::decay_t<AccumulatorFn>>{
                std::forward<Result>(initial_value),
                std::forward<AccumulatorFn>(accumulator)});
  }

  template <typename Result, scan_accumulator<Result, Type> AccumulatorFn>
      auto scan(Result&& initial_value, AccumulatorFn&& accumulator) &&
        requires is_header_included<scan_tag, Result, AccumulatorFn>
  {

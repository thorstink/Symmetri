...
struct member_overload<Type, SpecificObservable, filter_tag> {
  /**
   * \brief Emit only those items from an Observable that satisfies a provided
   predicate
   *
   * \marble filter
   {
   ...
   *
   * \details Actually this operator just checks if predicate returns true, then
   forwards emission
   *
   * \param predicate is predicate used to check emitted items. true -> items
   satisfies condition, false -> not
   * \return new specific_observable with the Filter operator as most recent
   operator.
    ...
    requires is_header_included<filter_tag, Predicate>
  {
    return static_cast<const SpecificObservable*>(this)->template lift<Type>(
        filter_impl<Type, std::decay_t<Predicate>>{
        ...
        requires is_header_included<filter_tag, Predicate>
  {
    return std::move(*static_cast<SpecificObservable*>(this))
        .template lift<Type>(filter_impl<Type, std::decay_t<Predicate>>{
            std::forward<Predicate>(predicate)});
  }
};
}  // namespace rpp::details

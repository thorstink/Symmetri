...
struct member_overload<Type, SpecificObservable, observe_on_tag> {
  /**
   * \brief Emit emissions of observable starting from this point via provided
   * scheduler
   *
   * \details Actually this operator just schedules emissions via provided
   * scheduler. So, actually it is delay(0) operator
   *
   * \param scheduler is scheduler used for scheduling of OnNext
   * \return new specific_observable with the observe_on operator as most recent
   * operator. \warning #include <rpp/operators/observe_on.hpp>
   *
   * \par Example:
  ...
  {
    return cast_this()->delay(schedulers::duration{0},
                              std::forward<TScheduler>(scheduler));
  }

  template <schedulers::constraint::scheduler TScheduler>
      auto observe_on(TScheduler&& scheduler) &&
        requires is_header_included<observe_on_tag, TScheduler>
  {
    return move_this().delay(schedulers::duration{0},
                             std::forward<TScheduler>(scheduler));
  }

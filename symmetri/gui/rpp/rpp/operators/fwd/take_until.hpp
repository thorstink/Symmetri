...
template <constraint::decayed_type Type,
          constraint::observable TTriggerObservable>
struct take_until_impl;

template <constraint::decayed_type Type, typename SpecificObservable>
struct member_overload<Type, SpecificObservable, take_until_tag> {
  /**
   * \brief Discard any items emitted by an Observable after a second Observable
   emits an item or terminates.
   * \warning The take_until subscribes and begins mirroring the source
   Observable. It also monitors a second Observable that you provide. If this
   second Observable emits an item or sends a on_error/on_completed
   notification, the Observable returned by take_until stops mirroring the
   source Observable and terminates.
   *
   * \marble take_until
   {
   ...
   *
   * \details Actually this operator just subscribes on 2 observables and
   completes original when `until_observable` emits any value
   *
   * \param until_observable is the observables that stops the source observable
   ...
   event.
   * \return new specific_observable with the take_until operator as most recent
   operator.
    ...
    requires is_header_included<take_until_tag, TTriggerObservable>
  {
    return cast_this()->template lift<Type>(
        take_until_impl<Type, std::decay_t<TTriggerObservable>>{
            std::forward<TTriggerObservable>(until_observable)});
  }

  template <constraint::observable TTriggerObservable>
      auto take_until(TTriggerObservable&& until_observable) &&
        requires is_header_included<take_until_tag, TTriggerObservable>
  {

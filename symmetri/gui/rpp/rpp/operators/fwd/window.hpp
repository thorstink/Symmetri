...
struct member_overload<Type, SpecificObservable, window_tag> {
  /**
   * \brief Subdivide original observable into sub-observables (windowed
   ...
   *
   * \return new specific_observable with the window operator as most recent
   operator.
   ...
   * - <b>OnNext</b>
   *    - Emits new window-observable if previous observable emitted requested
   amound of emisions
  ...
  auto window(size_t window_size) const&
    requires is_header_included<window_tag, Args...>
  {
    return window_impl<Type>(*static_cast<const SpecificObservable*>(this),
                             window_size);
  }
  ...
  template <typename... Args>
      auto window(size_t window_size) &&
        requires is_header_included<window_tag, Args...>
  {
    return window_impl<Type>(std::move(*static_cast<SpecificObservable*>(this)),
                             window_size);
  }
};
}  // namespace rpp::details

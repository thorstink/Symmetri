...
struct member_overload<Type, SpecificObservable, subscribe_on_tag> {
   ...
   * \return new specific_observable with the subscribe_on operator as most
   * recent operator. \warning #include <rpp/operators/subscribe_on.hpp>
   *
   * \par Example:
  ...
  {
    return subscribe_on_impl<Type>(
        *static_cast<const SpecificObservable*>(this), scheduler);
  }

  template <schedulers::constraint::scheduler TScheduler>
      auto subscribe_on(const TScheduler& scheduler) &&
        requires is_header_included<subscribe_on_tag, TScheduler>
  {
    return subscribe_on_impl<Type>(
        std::move(*static_cast<SpecificObservable*>(this)), scheduler);
  }
};
}  // namespace rpp::details

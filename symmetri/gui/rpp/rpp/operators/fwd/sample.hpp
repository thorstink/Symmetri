...
struct member_overload<Type, SpecificObservable, sample_tag> {
  /**
   * \brief Emit most recent emitted from original observable emission obtained
   during last period of time.
   * \details Emit item immediately in case of completion of the original
   observable
   *
   ...
   *
   * \details Actually operator just schedules periodical action and on each
   schedulable execution just emits last emitted emission (if any)
   *
   * \param period sampling period
   * \scheduler scheduler to use to schedule emissions with provided sampling
   period
   * \return new specific_observable with the sample_with_time operator as most
   recent operator.
  ...
  auto sample_with_time(schedulers::duration period,
                        const TScheduler& scheduler) const&
    requires is_header_included<sample_tag, TScheduler, Args...>
  {
    return static_cast<const SpecificObservable*>(this)->template lift<Type>(
        sample_with_time_impl<Type, TScheduler>{period, scheduler});
  }

...
concept reduce_accumulator =
    std::is_invocable_r_v<std::decay_t<Result>, Fn, std::decay_t<Result>,
                          std::decay_t<Type>>;
...
concept is_can_be_averaged =
    is_can_be_summed<T> && requires(CastBeforeDrop nt) {
                             { nt / size_t{} };
                           };
...
struct member_overload<Type, SpecificObservable, reduce_tag> {
  /**
   * \brief Applies accumulator function to each emission from observable and
   result of accumulator from previous step and emits final value
   *
   * \marble reduce
   {
   ...
   *
   * \details Actually this operator behaves like `scan()` + `take_last(1)`, so,
   it just accumulates `seed` and emits it `on_completed`
   *
   * \param initial_seed initial value for seed which will be applied for first
   value from observable. Then it will be replaced with result and etc.
   ...
   *
   * \return new specific_observable with the reduce operator as most recent
   operator.
   ...
   */
  template <typename Seed, reduce_accumulator<Seed, Type> AccumulatorFn,
            std::invocable<Seed&&> ResultSelectorFn = std::identity>
  auto reduce(Seed&& initial_seed, AccumulatorFn&& accumulator,
              ResultSelectorFn&& result_selector = {}) const&
    requires is_header_included<reduce_tag, Seed, AccumulatorFn,
                                ResultSelectorFn>
  {
    return static_cast<const SpecificObservable*>(this)
        ->template lift<utils::decayed_invoke_result_t<ResultSelectorFn,
                                                       std::decay_t<Seed>>>(
            reduce_impl<Type, std::decay_t<Seed>, std::decay_t<AccumulatorFn>,
                        std::decay_t<ResultSelectorFn>>{
   ...
   *
   * \tparam CastBeforeDivide cast accumulated value to this type before
   division
   * \return new specific_observable with the average operator as most recent
   operator.
   * \throws rpp::utils::not_enough_emissions in case of no any emissions from
   original observable
   *
   * \warning #include <rpp/operators/reduce.hpp>
   *
  ...
  auto average() const&
    requires(is_header_included<reduce_tag, CastBeforeDivide, Args...> &&
             is_can_be_averaged<Type, CastBeforeDivide>)
  {
    return average_impl<CastBeforeDivide>(
        *static_cast<const SpecificObservable*>(this));
  }
  ...
  template <typename CastBeforeDivide = Type, typename... Args>
      ...
      requires(is_header_included<reduce_tag, CastBeforeDivide, Args...>&&
                   is_can_be_averaged<Type, CastBeforeDivide>) {
        return average_impl<CastBeforeDivide>(
            std::move(*static_cast<SpecificObservable*>(this)));
      }
       ...
       *
       * \return new specific_observable with the sum operator as most recent
       operator.
       * \throws rpp::utils::not_enough_emissions in case of no any emissions
       from original observable
       *
       * \warning #include <rpp/operators/reduce.hpp>
       *
       * \par Example
  ...
  template <typename... Args>
      auto sum() &&
      requires(
          is_header_included<reduce_tag, Args...>&& is_can_be_summed<Type>) {
       ...
       *
       * \return new specific_observable with the count operator as most recent
       operator.
       * \warning #include <rpp/operators/reduce.hpp>
       *
       * \par Example
   ...
   * \return new specific_observable with the min operator as most recent
   operator.
   * \throws rpp::utils::not_enough_emissions in case of no any emissions from
   original observable
   *
   ...
   */
  template <std::strict_weak_order<Type, Type> Comparator = std::less<Type>,
            typename... Args>
  auto min(Comparator&& comparator = {}) const&
    requires is_header_included<reduce_tag, Comparator, Args...>
  {
    return min_impl(*static_cast<const SpecificObservable*>(this),
                    std::forward<Comparator>(comparator));
  }

  template <std::strict_weak_order<Type, Type> Comparator = std::less<Type>,
            typename... Args>
      auto min(Comparator&& comparator = {}) &&
        requires is_header_included<reduce_tag, Comparator, Args...>
  {
    return min_impl(std::move(*static_cast<SpecificObservable*>(this)),
                    std::forward<Comparator>(comparator));
  }
   ...
   *
   * \param comparator is function to deduce if left value is less than right
   * \return new specific_observable with the max operator as most recent
   operator.
   * \throws rpp::utils::not_enough_emissions in case of no any emissions from
   original observable
   *
  ...
  template <std::strict_weak_order<Type, Type> Comparator = std::less<Type>,
            typename... Args>
  auto max(Comparator&& comparator = {}) const&
    requires is_header_included<reduce_tag, Comparator, Args...>
  {
    return max_impl(*static_cast<const SpecificObservable*>(this),
                    std::forward<Comparator>(comparator));
  }

  template <std::strict_weak_order<Type, Type> Comparator = std::less<Type>,
            typename... Args>
      auto max(Comparator&& comparator = {}) &&
        requires is_header_included<reduce_tag, Comparator, Args...>
  {
    return max_impl(std::move(*static_cast<SpecificObservable*>(this)),
                    std::forward<Comparator>(comparator));
  }
};
}  // namespace rpp::details

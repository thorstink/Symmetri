...
struct member_overload<Type, SpecificObservable, take_while_tag> {
  /**
   * \brief Sends items provided by observable while items are satisfy
   predicate. When condition becomes false -> sends `on_completed`
   *
   * \marble take_while
   {
   ...
   *
   * \details Actually this operator just emits values while predicate returns
   true
   *
   * \param predicate is predicate used to check items
   * \return new specific_observable with the take_while operator as most recent
   operator.
        ...
        requires is_header_included<take_while_tag, Predicate>
  {
    return std::move(*static_cast<SpecificObservable*>(this))
        .template lift<Type>(take_while_impl<Type, std::decay_t<Predicate>>{
            std::forward<Predicate>(predicate)});
  }
};
}  // namespace rpp::details

...
struct member_overload<Type, SpecificObservable, repeat_tag> {
  /**
  * \brief Re-subscribes on current observable provided amount of times when
  `on_completed` obtained
  *
  * \marble repeat
    {
  ...
  *
  * \details Actually this operator re-subscribes on same observable when
  `on_completed` obtained while counter not reached zero
  *
  ...
  *  - `count(10)` - 1 normal subscription and 9 re-subscriptions during
  on_completed
  * \return new specific_observable with the repeat operator as most recent
  operator.
  ...
  */
  ...
  auto repeat(size_t count) const&

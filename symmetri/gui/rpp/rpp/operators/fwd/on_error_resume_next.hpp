...
concept resume_callable =
    std::invocable<Fn, std::exception_ptr> &&
    constraint::observable<
        utils::decayed_invoke_result_t<Fn, std::exception_ptr>>;
...
struct member_overload<Type, SpecificObservable, on_error_resume_next_tag> {
  /**
   * \brief Recover from an on_error notification by continuing the sequence
   without error.
   * \details The operator intercepts an `on_error` notification from the source
   Observable and, instead of passing it through to any observers, replaces it
   with some other item or sequence of items.
   * \warning This operator potentially allows the resulting Observable to
   terminate normally or not to terminate at all.
   *
   ...
   *
   * \details Actually this operator just subscribes on observable calculated
   from callback when `on_error` from original observable obtained.
   *
   * \param resume_callable A callable that is given an error pointer and shall
   return an Observable.
   * \return new specific_observable with the on_error_resume_next operator as
  ...
  auto on_error_resume_next(ResumeCallable&& resume_callable) const&
    requires is_header_included<on_error_resume_next_tag, ResumeCallable>
  {
    return cast_this()->template lift<Type>(
        on_error_resume_next_impl<Type, std::decay_t<ResumeCallable>>{
            std::forward<ResumeCallable>(resume_callable)});
  }

  template <rpp::details::resume_callable ResumeCallable>
      auto on_error_resume_next(ResumeCallable&& resume_callable) &&
        requires is_header_included<on_error_resume_next_tag, ResumeCallable>
  {

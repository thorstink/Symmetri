...
struct member_overload<Type, SpecificObservable, distinct_until_changed_tag> {
  /**
   * \brief Suppress consecutive duplicates of emissions from original
   observable
   *
   ...
   *
   * \details Actually this operator has `std::optional` with last item and
   checks everytime where new emission is same or not.
   *
   * \param equality_fn optional equality comparator function
   * \return new specific_observable with the distinct_until_changed operator as
   ...
   */
  template <
      std::equivalence_relation<Type, Type> EqualityFn = std::equal_to<Type>>
  auto distinct_until_changed(EqualityFn&& equality_fn = EqualityFn{}) const&
    requires is_header_included<distinct_until_changed_tag, EqualityFn>
  {
    return static_cast<const SpecificObservable*>(this)->template lift<Type>(
        distinct_until_changed_impl<Type, std::decay_t<EqualityFn>>{
            std::forward<EqualityFn>(equality_fn)});
  }

  template <
      std::equivalence_relation<Type, Type> EqualityFn = std::equal_to<Type>>
      auto distinct_until_changed(EqualityFn&& equality_fn = EqualityFn{}) &&
        requires is_header_included<distinct_until_changed_tag, EqualityFn>
  {

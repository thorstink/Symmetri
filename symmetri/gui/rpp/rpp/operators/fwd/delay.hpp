...
struct member_overload<Type, SpecificObservable, delay_tag> {
  /**
   * \brief Shift the emissions from an Observable forward in time by a
   particular amount.
   * \details The delay operator modifies its source Observable by pausing for a
   ...
   source Observable’s items. This has the effect of shifting the entire
   sequence of items emitted by the Observable forward in time by that specified
   increment.
   *
   * \marble delay
   {
   ...
   * \return new specific_observable with the delay operator as most recent
   operator.
    ...
    requires is_header_included<delay_tag, TScheduler>
  {
    return cast_this()->template lift<Type>(
        delay_impl<Type, std::decay_t<TScheduler>>{
            std::forward<TScheduler>(scheduler),
            std::chrono::duration_cast<rpp::schedulers::duration>(
                delay_duration)});
  }

  template <schedulers::constraint::scheduler TScheduler>
      auto delay(auto&& delay_duration, TScheduler&& scheduler) &&
        requires is_header_included<delay_tag, TScheduler>
  {

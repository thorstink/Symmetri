...
struct member_overload<Type, SpecificObservable, take_last_tag> {
  /**
   * \brief Emit only last `count` items provided by observable, then send
   `on_completed`
   *
   * \marble take_last
   {
   ...
   *
   * \details Actually this operator has buffer of requested size inside, keeps
   last `count` values and emit stored values on `on_completed`
   *
   * \param count amount of last items to be emitted
   * \return new specific_observable with the take_last operator as most recent
   operator.
  ...
  auto take_last(size_t count) const&
        ...
        requires is_header_included<take_last_tag, Args...>
  {
    return std::move(*static_cast<SpecificObservable*>(this))
        .template lift<Type>(take_last_impl<Type>{count});
  }
};
}  // namespace rpp::details

...
struct member_overload<Type, SpecificObservable, buffer_tag> {
  ...
  * \return new specific_observable with the buffer operator as most recent
  operator.
  ...
  * - <b>On subscribe</b>
  *    - Allocates one `shared_ptr` to store `std::vector<Type>` of requested
  size.
  * - <b>OnNext</b>
  *    - Accumulates emissions inside current bundle and emits this bundle when
  requested cound reached and starts new bundle.
  * - <b>OnError</b>
  ...
  */
  ...
  auto buffer(size_t count) const&
    requires is_header_included<buffer_tag, Args...>
  {
    return CastThis()->template lift<buffer_bundle_type<Type>>(
  ...
  template <typename... Args>
      auto buffer(size_t count) &&
        requires is_header_included<buffer_tag, Args...>
  {
    return MoveThis().template lift<buffer_bundle_type<Type>>(
        buffer_impl<Type>{count});
  }

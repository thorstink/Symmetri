...
struct sample_with_time_impl {
  ...
  auto operator()(TSub&& in_subscriber) const {
    auto state = std::make_shared<sample_state_with_serialized_spinlock<Type>>(
    ...
    scheduler.create_worker(state->children_subscriptions)
        .schedule(period,
                  [period = period, subscriber = subscriber,
                   state]() -> rpp::schedulers::optional_duration {
    ...
    return create_subscriber_with_state<Type>(
        state->children_subscriptions, sample_on_next{}, sample_on_error{},
        sample_on_completed{}, std::move(subscriber), std::move(state));
  }
};
}  // namespace rpp::details

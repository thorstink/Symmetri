...
struct take_last_state {
  explicit take_last_state(size_t count) : items(count) {}
...
struct take_last_on_next {
  ...
  void operator()(T&& v, const auto&,
                  const take_last_state<Type>& state) const {
    // handle case "count==0"
    if (state.items.empty()) return;

    state.items[state.current_end_position].emplace(std::forward<T>(v));
    state.current_end_position =
        state.get_next_position(state.current_end_position);
  }
};
...
struct take_last_impl {
  ...
  auto operator()(TSub&& subscriber) const {
    ...
    return create_subscriber_with_dynamic_state<Type>(
        std::move(subscription), take_last_on_next{},
        utils::forwarding_on_error{}, take_last_on_completed{},
        std::forward<TSub>(subscriber), take_last_state<Type>{count});
  }
};
}  // namespace rpp::details

...
using window_toggle_observable =
    decltype(std::declval<rpp::operators::details::forwarding_subject<Type>>()
                 .get_observable());
}  // namespace rpp
...
struct window_toggle_state {
  ...
  window_toggle_state(TObserver&& observer, const TClosingsSelectorFn& closings)
      : m_state{state_t{std::move(observer)}}, m_closings{closings} {}
  ...
  auto on_new_subject(const Subject& subject) {
    ...
    const auto itr = locked_state->observers.insert(
        locked_state->observers.cend(), subject.get_observer());
...
struct window_toggle_closing_observer_strategy {
  ...
  decltype(std::declval<TState>().on_new_subject(
      std::declval<typename TState::Subject>())) itr;

  void on_next(const auto&) const { on_completed(); }

  void on_error(const std::exception_ptr& err) const {
    auto locked_state = state->get_state_under_lock();
    for (const auto& obs : locked_state->observers) obs.on_error(err);
...
struct window_toggle_opening_observer_strategy {
  ...
  void on_next(T&& v) const {
    ...
    state->get_closing(std::forward<T>(v))
        .subscribe(window_toggle_closing_observer_strategy<TState>{
            disposable, state, subject.get_disposable(), itr});
  }
...
template <rpp::constraint::observer TObserver,
          rpp::constraint::observable TOpeningsObservable,
          typename TClosingsSelectorFn>
  requires rpp::constraint::observable<std::invoke_result_t<
      TClosingsSelectorFn,
      rpp::utils::extract_observable_type_t<TOpeningsObservable>>>
class window_toggle_observer_strategy {
 ...
 public:
  ...
  window_toggle_observer_strategy(TObserver&& observer,
                                  const TOpeningsObservable& openings,
                                  const TClosingsSelectorFn& closings)
      : m_state{std::make_shared<TState>(std::move(observer), closings)} {
    m_state->get_state_under_lock()->observer.set_upstream(
        m_disposable->add_ref());
    m_disposable->add(openings.subscribe_with_disposable(
        window_toggle_opening_observer_strategy<TState>{m_disposable,
                                                        m_state}));
  }

  void on_next(const auto& v) const {
    const auto locked_state = m_state->get_state_under_lock();
    for (const auto& obs : locked_state->observers) obs.on_next(v);
  }
  ...
  void on_completed() const {
    const auto locked_state = m_state->get_state_under_lock();
    for (const auto& obs : locked_state->observers) obs.on_completed();
 ...
 private:
  std::shared_ptr<rpp::refcount_disposable> m_disposable =
      disposable_wrapper_impl<rpp::refcount_disposable>::make().lock();
  std::shared_ptr<TState> m_state;
};

template <rpp::constraint::observable TOpeningsObservable,
          typename TClosingsSelectorFn>
  requires rpp::constraint::observable<std::invoke_result_t<
      TClosingsSelectorFn,
      rpp::utils::extract_observable_type_t<TOpeningsObservable>>>
struct window_toggle_t
    : lift_operator<window_toggle_t<TOpeningsObservable, TClosingsSelectorFn>,
                    TOpeningsObservable, TClosingsSelectorFn> {
  using lift_operator<window_toggle_t<TOpeningsObservable, TClosingsSelectorFn>,
                      TOpeningsObservable, TClosingsSelectorFn>::lift_operator;
  ...
  struct operator_traits {
    ...
    using observer_strategy = window_toggle_observer_strategy<
        std::decay_t<TObserver>, TOpeningsObservable, TClosingsSelectorFn>;
  };
 ...
 * @brief Subdivide original observable into sub-observables (window
 observables) and emit sub-observables of items instead of original items
 * @details Values from `openings` observable used to specify moment when new
 window will be opened. `closings_selector` is used to obtain observable to
 specify moment when new window will be closed.
 *
 ...
 * @param closings_selector is function which returns observable which
 emission/completion means closing of opened window
 *
...
template <rpp::constraint::observable TOpeningsObservable,
  ...
  requires rpp::constraint::observable<std::invoke_result_t<
      TClosingsSelectorFn,
      rpp::utils::extract_observable_type_t<TOpeningsObservable>>>
auto window_toggle(TOpeningsObservable&& openings,
                   TClosingsSelectorFn&& closings_selector) {
  return details::window_toggle_t<std::decay_t<TOpeningsObservable>,
                                  std::decay_t<TClosingsSelectorFn>>{
      std::forward<TOpeningsObservable>(openings),
      std::forward<TClosingsSelectorFn>(closings_selector)};
}
}  // namespace rpp::operators

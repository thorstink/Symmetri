...
struct start_with_t {
 ...
 private:
  template <rpp::constraint::observable TObservable>
  static auto apply(TObservable&& observable,
                    const TObservables&... observables) {
    return rpp::source::concat(observables...,
                               std::forward<TObservable>(observable));
  }
};

template <rpp::constraint::decayed_type PackedContainer,
...
/**
 * @brief Combines submissions from current observable with other observables
 into one but without overlapping and starting from observables provided as
 arguments
 * @warning If by some reason you need to interpet observables as "values", not
 sources of data, then use `start_with_values` instead
 *
 ...
 *
 * @details Actually it makes concat(observables_to_start_with...,
 current_observable) so observables from argument subscribed before current
 observable
 *
...
auto start_with(TObservable&& observable, TObservables&&... observables) {
  return details::start_with_t<std::decay_t<TObservable>,
                               std::decay_t<TObservables>...>{
...
/**
 * @brief Combines submissions from current observable with values into one but
 without overlapping and starting from values provided as arguments
 *
 ...
 *
 * @details Actually it makes concat(rpp::source::just(vals_to_start_with)...,
 current_observable) so observables from argument subscribed before current
 observable
 * @details This overloading operates on rpp::schedulers::current_thread by
 default
 *
...
auto start_with_values(T&& v, Ts&&... vals) {
  return start_with_values<MemoryModel>(
      rpp::schedulers::defaults::iteration_scheduler{}, std::forward<T>(v),
      std::forward<Ts>(vals)...);
}
 ...
 */
template <constraint::memory_model MemoryModel /* = memory_model::use_stack */,
          rpp::schedulers::constraint::scheduler TScheduler, typename T,
          typename... Ts>
  requires(rpp::constraint::decayed_same_as<T, Ts> && ...)
auto start_with_values(const TScheduler& scheduler, T&& v, Ts&&... vals) {
  ...
  using container = std::conditional_t<
      ...
      rpp::details::shared_container<inner_container>>;

  return details::start_with_values_t<container, TScheduler>{
      scheduler, std::forward<T>(v), std::forward<Ts>(vals)...};
}

template <constraint::memory_model MemoryModel /* = memory_model::use_stack*/,
          typename T, typename... Ts>
  requires((rpp::constraint::decayed_same_as<T, Ts> && ...) &&
           !(rpp::constraint::observable<T> ||
             (rpp::constraint::observable<Ts> || ...)))
auto start_with(T&& v, Ts&&... vals) {
  return start_with_values<MemoryModel>(std::forward<T>(v),
                                        std::forward<Ts>(vals)...);
}
...
auto start_with(const TScheduler& scheduler, T&& v, Ts&&... vals) {
  return start_with_values<MemoryModel>(scheduler, std::forward<T>(v),
                                        std::forward<Ts>(vals)...);
}
}  // namespace rpp::operators

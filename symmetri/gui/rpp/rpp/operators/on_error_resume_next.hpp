...
struct on_error_resume_next_disposable final
    : public rpp::composite_disposable {
  on_error_resume_next_disposable(TObserver&& observer)
      : rpp::composite_disposable{}, observer(std::move(observer)) {}
...
struct on_error_resume_next_observer_strategy {
  ...
  on_error_resume_next_observer_strategy(TObserver&& observer,
      ...
      : state{init_state(std::move(observer))}, selector{selector} {}
  ...
  void on_error(const std::exception_ptr& err) const {
    try {
      selector(err).subscribe(
          on_error_resume_next_inner_observer_strategy<TObserver>{
              std::shared_ptr<TObserver>(state, &state->observer)});
    } catch (...) {
  ...
  static std::shared_ptr<on_error_resume_next_disposable<TObserver>> init_state(
      TObserver&& observer) {
    const auto d = disposable_wrapper_impl<
        on_error_resume_next_disposable<TObserver>>::make(std::move(observer));
    auto ptr = d.lock();

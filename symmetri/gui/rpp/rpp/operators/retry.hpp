...
template <rpp::constraint::observer TObserver,
          constraint::decayed_type Observable>
struct retry_state_t final {
  retry_state_t(TObserver&& in_observer, const Observable& observable,
...
template <rpp::constraint::observer TObserver, typename TObservable>
void drain(
    const std::shared_ptr<retry_state_t<TObserver, TObservable>>& state) {
  while (!state->disposable.is_disposed()) {
    if (state->count) --state->count.value();
    ...
    try {
      ...
      state->observable.subscribe(
          observer<value_type, retry_observer_strategy<TObserver, TObservable>>{
              state});
...
struct retry_t {
  ...
  void subscribe(TObserver&& observer, TObservable&& observble) const {
    const auto ptr = std::make_shared<
        retry_state_t<std::decay_t<TObserver>, std::decay_t<TObservable>>>(
        std::forward<TObserver>(observer), std::forward<TObservable>(observble),
        count ? count.value() + 1 : count);
    drain(ptr);
  }
};
}  // namespace rpp::operators::details
...
/**
 * @brief The retry operator attempts to resubscribe to the observable when an
 error occurs, up to the specified number of retries.
 *
 * @marble retry
   {
       source observable    : +-1-#
 ...
 */
inline auto retry(size_t count) { return details::retry_t{count}; }

/**
* @brief The infinite retry operator continuously attempts to resubscribe to the
observable upon error, without a retry limit.
*
...
* @see https://reactivex.io/documentation/operators/retry.html
*/
inline auto retry() { return details::retry_t{}; }
}  // namespace rpp::operators

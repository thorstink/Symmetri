...
template <rpp::constraint::decayed_type OnNext,
          rpp::constraint::decayed_type OnError,
          rpp::constraint::decayed_type OnCompleted>
struct tap_t
    : public operators::details::lift_operator<
          tap_t<OnNext, OnError, OnCompleted>, OnNext, OnError, OnCompleted> {
  using operators::details::lift_operator<tap_t<OnNext, OnError, OnCompleted>,
                                          OnNext, OnError,
                                          OnCompleted>::lift_operator;

  template <rpp::constraint::decayed_type T>
  struct operator_traits {
    static_assert(rpp::constraint::invocable_r_v<void, OnNext, T>,
                  "OnNext is not invocable with T");
...
/**
 * @brief Register callbacks to inspect observable emissions and perform
 * side-effects
 *
 ...
 */
template <std::invocable<const std::exception_ptr&>
              OnError /* = rpp::utils::empty_function_t<std::exception_ptr> */>
...
auto tap(OnError&& on_error) {
  ...
  return details::tap_t<std::decay_t<OnNext>, std::decay_t<OnError>,
                        std::decay_t<OnCompleted>>{
      OnNext{}, std::forward<OnError>(on_error), OnCompleted{}};
}
...
auto tap(OnCompleted&& on_completed) {
  ...
  return details::tap_t<std::decay_t<OnNext>, std::decay_t<OnError>,
                        std::decay_t<OnCompleted>>{
      OnNext{}, OnError{}, std::forward<OnCompleted>(on_completed)};
}
...
auto tap(OnNext&& on_next, OnCompleted&& on_completed) {
  ...
  return details::tap_t<std::decay_t<OnNext>, std::decay_t<OnError>,
                        std::decay_t<OnCompleted>>{
      std::forward<OnNext>(on_next), OnError{},
...
template <typename OnNext /* = rpp::utils::empty_function_any_t */,
          std::invocable<const std::exception_ptr&>
              OnError /* = rpp::utils::empty_function_t<std::exception_ptr> */,
          std::invocable<> OnCompleted /* = rpp::utils::empty_function_t<> */>
  requires utils::is_not_template_callable<OnError>

...
struct with_latest_from_on_next_inner {
  void operator()(auto&& value, const constraint::subscriber auto&,
...
void with_latest_from_subscribe(const auto& state_ptr, const TObs& observable,
                                const auto& subscriber) {
  using Type = utils::extract_observable_type_t<TObs>;
  observable.subscribe(create_subscriber_with_state<Type>(
      state_ptr->children_subscriptions.make_child(),
      with_latest_from_on_next_inner<I>{}, with_latest_from_on_error{},
      [](const auto&, const auto&) {}, subscriber, state_ptr));
}
...
void with_latest_from_subscribe_observables(std::index_sequence<I...>,
  ...
  (with_latest_from_subscribe<I>(state_ptr, std::get<I>(observables_tuple),
                                 subscriber),
   ...);
}
...
struct with_latest_from_on_next_outer {
  ...
  void operator()(T&& v, const auto& sub, const auto& state) const {
    using ResultType =
        utils::decayed_invoke_result_t<TSelector, std::decay_t<T>,
                                       ValueTypes...>;

    auto result = std::apply(
        [&](const auto&... current_cached_vals) -> std::optional<ResultType> {
          auto lock = std::apply(
              [](auto&... mutexes) { return std::scoped_lock{mutexes...}; },
              state->mutexes);

          if ((current_cached_vals.has_value() && ...))
            return state->selector(
                utils::as_const(std::forward<T>(v)),
                utils::as_const(current_cached_vals.value())...);
          return std::nullopt;
        },
        state->vals);

    if (result.has_value()) sub.on_next(std::move(result.value()));
  }
};

template <typename TSelector, constraint::decayed_type... ValueTypes>
struct with_latest_from_state_with_serialized_spinlock
    : public with_latest_from_state<TSelector, ValueTypes...> {
  using with_latest_from_state<TSelector,
                               ValueTypes...>::with_latest_from_state;

  // we can use spinlock there because 99.9% of time only one ever thread would
  // send values from on_next (main observable), but we have small probability
  // to get error from inner observables immediately
...
struct with_latest_from_impl {
  ...
  auto operator()(TSub&& in_subscriber) const {
    auto state =
        std::make_shared<with_latest_from_state_with_serialized_spinlock<
            TSelector, utils::extract_observable_type_t<TObservables>...>>(
            selector, in_subscriber.get_subscription());
    ...
    with_latest_from_subscribe_observables(
        std::index_sequence_for<TObservables...>{}, state, subscriber,
        observables);
    ...
    return create_subscriber_with_state<Type>(
        ...
        with_latest_from_on_next_outer<
            TSelector, utils::extract_observable_type_t<TObservables>...>{},
        with_latest_from_on_error{}, with_latest_from_on_completed_outer{},

...
struct concat_on_next_outer {
  ...
  void operator()(TObs&& new_observable, const TSub& sub,
    ...
    if (state->inner_subscribed.exchange(true, std::memory_order::acq_rel)) {
      ...
      if (state->inner_subscribed.exchange(true, std::memory_order::relaxed)) {
        state->observables_to_subscribe.push(
            std::forward<TObs>(new_observable).as_dynamic());
        return;
      }
    }
 ...
 private:
  static void subscribe_inner_subscriber(
      const auto& observable, const constraint::subscriber auto& subscriber,
      const std::shared_ptr<concat_state<ValueType>>& state) {
    observable.subscribe(create_subscriber_with_state<ValueType>(
        state->children_subscriptions.make_child(), concat_on_next_inner{},
        concat_on_error{},
...
struct concat_impl {
  ...
  auto operator()(TSub&& in_subscriber) const {
    auto state =
        std::make_shared<concat_state_with_serialized_spinlock<ValueType>>(
    ...
    return create_subscriber_with_state<Type>(
        state->source_subscription, concat_on_next_outer<ValueType>{},
        concat_on_error{}, concat_on_completed<ValueType>{},
...
auto concat_with_impl(TObservables&&... observables) {
  return source::just(rpp::schedulers::immediate{},

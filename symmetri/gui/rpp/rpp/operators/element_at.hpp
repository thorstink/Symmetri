  ...
  void on_completed() const {
    if (count) {
      observer.on_error(std::make_exception_ptr(
          utils::out_of_range{"index is out of bounds"}));
    } else {
      observer.on_completed();
    }
  }
...
/**
 * @brief Emit item located at specified `index` location in the sequence of
 ...
 * @details If source observable completes without emitting at least `index` + 1
 items, observable emits an error
 *

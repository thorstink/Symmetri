...
/**
 * @brief For each item from this observable, filter out repeated values and
 emit only items that have not already been emitted
 *
 * @marble distinct
 {
 ...
 *
 * @warning This operator keeps an `std::unordered_set<T>` of past values, so
 std::hash<T> specialization is required.
 *
 * @ingroup filtering_operators
 * @see https://reactivex.io/documentation/operators/distinct.html
 */
inline auto distinct() { return details::distinct_t{}; }
}  // namespace rpp::operators

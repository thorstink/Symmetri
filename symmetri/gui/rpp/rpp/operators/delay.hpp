...
class queue_based_worker final
    : public std::enable_shared_from_this<
          queue_based_worker<T, Subscriber, Worker>> {
 public:
  queue_based_worker(schedulers::duration delay, Worker&& worker,
                     const Subscriber& subscriber)
      : m_delay{delay}, m_worker{std::move(worker)}, m_subscriber{subscriber} {}
  ...
  struct on_next {
    void operator()(
        auto&& value,
        const std::shared_ptr<queue_based_worker<T, Subscriber, Worker>>& state)
        const {
 ...
 private:
  ...
  void emplace(TT&& item) {
    if (const auto timepoint = emplace_safe(std::forward<TT>(item))) {
      m_worker.schedule(
          timepoint.value(),
          [state =
               this->shared_from_this()]() -> schedulers::optional_duration {
  ...
  std::optional<schedulers::time_point> emplace_safe(TT&& item) {
    ...
    const auto delay = std::is_same_v<std::exception_ptr, std::decay_t<TT>>
                           ? schedulers::duration{0}
                           : m_delay;
    m_queue.emplace(++m_current_id, m_worker.now() + delay,
                    std::forward<TT>(item));
  ...
  schedulers::optional_duration drain_queue() {
    while (true) {
      ...
      if (top.time > now) return top.time - now;
      ...
      std::visit(
          utils::overloaded{[&](T&& v) { m_subscriber.on_next(std::move(v)); },
                            [&](const std::exception_ptr& err) {
                              m_subscriber.on_error(err);
                            },
 ...
 private:
  struct emission {
    ...
    emission(size_t id, schedulers::time_point time, TT&& item)
        : id{id}, time{std::move(time)}, item{std::forward<TT>(item)} {}
    ...
    bool operator<(const emission& other) const {
      return std::tie(time, id) >= std::tie(other.time, other.id);
    }
  };
...
struct delay_impl {
  ...
  auto operator()(TSub&& subscriber) const {
    ...
    using state_t = queue_based_worker<Type, std::decay_t<TSub>,
                                       std::decay_t<decltype(worker)>>;
    auto state = std::make_shared<state_t>(delay, std::move(worker),
                                           std::forward<TSub>(subscriber));

    return create_subscriber_with_state<Type>(
        std::move(subscription), typename state_t::on_next{},
        typename state_t::on_error{}, typename state_t::on_completed{},
        std::move(state));
  }
};
}  // namespace rpp::details

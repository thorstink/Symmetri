...
/**
 * @brief Converts ordinary observable to rpp::connectable_observable with help
 * of provided subject
 * @details Connectable observable is common observable, but actually it starts
 * emissions of items only after call "connect", "ref_count" or any other
 * available way. Also it uses subject to multicast values to subscribers
 * @warning Same subject would be used to all observables lifted via this
...
auto multicast(Subject&& subject) {
  return details::multicast_t<std::decay_t<Subject>>{
      std::forward<Subject>(subject)};
}

/**
 * @brief Converts ordinary observable to rpp::connectable_observable with help
 * of inline instsantiated subject of provided type
 * @details Connectable observable is common observable, but actually it starts
 * emissions of items only after call "connect", "ref_count" or any other
 * available way. Also it uses subject to multicast values to subscribers
 * @warning This overloading creates fresh `Subject<Type>` everytime new
 * observable passed to this operator
 *

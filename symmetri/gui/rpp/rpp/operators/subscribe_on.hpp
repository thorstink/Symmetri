...
auto subscribe_on_impl(TObs&& obs, const TScheduler& scheduler) {
  return source::create<Type>(
      [obs = std::forward<TObs>(obs),
       scheduler]<constraint::subscriber_of_type<Type> TSub>(
          TSub&& subscriber) {
        auto worker = scheduler.create_worker(subscriber.get_subscription());
        worker.schedule(
            [obs, subscriber = std::forward<TSub>(
                      subscriber)]() mutable -> schedulers::optional_duration {

...
template <rpp::constraint::observer TObserver,
          rpp::constraint::decayed_type Seed, rpp::constraint::decayed_type Fn>
...
template <rpp::constraint::decayed_type InitialValue,
          rpp::constraint::decayed_type Fn>
struct scan_t : lift_operator<scan_t<InitialValue, Fn>, InitialValue, Fn> {
  using operators::details::lift_operator<scan_t<InitialValue, Fn>,
                                          InitialValue, Fn>::lift_operator;

  template <rpp::constraint::decayed_type T>
  struct operator_traits {
    static_assert(
        std::is_invocable_r_v<InitialValue, Fn, InitialValue&&, T>,
    ...
    using observer_strategy =
        scan_observer_strategy<TObserver, InitialValue, Fn>;
  };
...
struct scan_no_seed_t : lift_operator<scan_no_seed_t<Fn>, Fn> {
  ...
  template <rpp::constraint::decayed_type T>
  struct operator_traits {
    static_assert(std::is_invocable_r_v<T, Fn, T&&, T>,
...
/**
 * @brief Apply accumulator function for each emission from observable and
 result of accumulator from previous step and emit (and cache) resulting value
 *
 * @marble scan
 {
 ...
 *
 * @details Actually this operator applies provided accumulator function to seed
 and new emission, emits resulting value and updates seed value for next
 emission
 * @warning Initial value would be used as first value from this observable
 (would be sent during subscription) and initial value for cache
 *
 * @par Performance notes:
 * - No any heap allocations at all
 * - Keep actual seed/cache inside observable and updating it every emission
 *
 * @param initial_value initial value for seed which would be sent during
 subscription and applied for first value from observable. Then it will be
 replaced with result and etc.
...
auto scan(InitialValue&& initial_value, Fn&& accumulator) {
  return details::scan_t<std::decay_t<InitialValue>, std::decay_t<Fn>>{
      std::forward<InitialValue>(initial_value), std::forward<Fn>(accumulator)};
}
 ...
 * @details Actually this operator applies provided accumulator function to seed
 and new emission, emits resulting value and updates seed value for next
 emission
 * @warning There is no initial value for seed, so, first value would be used as
...
auto scan(Fn&& accumulator) {
  return details::scan_no_seed_t<std::decay_t<Fn>>{
      std::forward<Fn>(accumulator)};
}
}  // namespace rpp::operators

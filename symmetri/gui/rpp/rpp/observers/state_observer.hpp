...
/**
 * \brief Special type of specific_observer which has some state which this
 * observer stores and pass to each callback. Actually it is base class for all
 * observers.
 */
template <constraint::decayed_type T, typename OnNext, typename OnError,
          typename OnCompleted, constraint::decayed_type... States>
...
class state_observer : public details::typed_observer_tag<T> {
 public:
  ...
  void on_next(const T& v) const {
    std::apply([&v, this](const States&... states) { m_on_next(v, states...); },
               m_state);
  }
  ...
  void on_next(T&& v) const {
    std::apply(
        [&v, this](const States&... states) {
          m_on_next(std::move(v), states...);
        },
        m_state);
  }
   ...
   * during generation next data. \warning Obtaining this call means no any
   * further on_next or on_completed calls \param err details of error
   */
  void on_error(const std::exception_ptr& err) const {
    std::apply(
        [&err, this](const States&... states) { m_on_err(err, states...); },
        m_state);
  }

  /**
   * \brief Observable calls this method to notify observer about finish of
   * work. \warning Obtaining this call means no any further on_next calls
   */
  void on_completed() const { std::apply(m_on_completed, m_state); }
...
state_observer(TOnNext, Args...)
    -> state_observer<std::decay_t<utils::function_argument_t<TOnNext>>,
                      TOnNext, Args...>;

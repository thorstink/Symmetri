...
class observer_vtable {
 ...
 protected:
  struct vtable_t {
    ...
    void (*const on_error_ptr)(const observer_vtable*,
                               const std::exception_ptr&){};
    void (*const on_completed_ptr)(const observer_vtable*){};

    void (*const set_upstream_ptr)(observer_vtable*,
                                   const disposable_wrapper&){};
    bool (*const is_disposed_ptr)(const observer_vtable*){};
  };

  observer_vtable(vtable_t&& vtable) : m_vtable{std::move(vtable)} {}

  const vtable_t m_vtable{};
};

template <rpp::constraint::observer TObs>
class type_erased_observer
 ...
 public:
  type_erased_observer(TObs&& observer)
      : Base{Vtable{
            .on_next_lvalue_ptr =
                +[](const Base* b, const Type& v) { cast(b).on_next(v); },
            .on_next_rvalue_ptr =
                +[](const Base* b, Type&& v) { cast(b).on_next(std::move(v)); },
            .on_error_ptr =
                +[](const Base* b, const std::exception_ptr& err) {
                  cast(b).on_error(err);
                },
            .on_completed_ptr = +[](const Base* b) { cast(b).on_completed(); },
            .set_upstream_ptr =
                +[](Base* b, const rpp::disposable_wrapper& d) {
                  cast(b).set_upstream(d);
                },
            .is_disposed_ptr =
                +[](const Base* b) { return cast(b).is_disposed(); }}},
...
class dynamic_strategy final {
 public:
  ...
  explicit dynamic_strategy(observer<Type, Strategy>&& obs)
      : m_observer{
            std::make_shared<type_erased_observer<observer<Type, Strategy>>>(
                std::move(obs))} {}

  void set_upstream(const disposable_wrapper& d) noexcept {
    m_observer->set_upstream(d);
  }

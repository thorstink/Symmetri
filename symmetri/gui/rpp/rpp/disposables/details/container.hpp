...
class static_disposables_container {
 public:
  ...
  static_disposables_container& operator=(
      const static_disposables_container& other) = delete;

  static_disposables_container& operator=(
      static_disposables_container&& other) noexcept {
  ...
  ~static_disposables_container() noexcept { clear(); }

  void push_back(const rpp::disposable_wrapper& d) {
    if (m_size >= Count)
      throw rpp::utils::more_disposables_than_expected{
          "static_disposables_container obtained more disposables than "
          "expected"};
  ...
  void remove(const rpp::disposable_wrapper& d) {
    for (size_t i = 0; i < m_size;) {
      ...
      for (size_t j = i + 1; j < m_size; ++j) *get(j - 1) = std::move(*get(j));
  ...
  void clear() {
    for (size_t i = 0; i < m_size; ++i) std::destroy_at(get(i));
    m_size = 0;
  }

 private:
  const rpp::disposable_wrapper* get(size_t i) const {
    return std::launder(reinterpret_cast<const rpp::disposable_wrapper*>(
        &m_data[i * sizeof(rpp::disposable_wrapper)]));
  }
  rpp::disposable_wrapper* get(size_t i) {
    return std::launder(reinterpret_cast<rpp::disposable_wrapper*>(
        &m_data[i * sizeof(rpp::disposable_wrapper)]));
  }

 private:
  alignas(rpp::disposable_wrapper)
      std::byte m_data[sizeof(rpp::disposable_wrapper) * Count]{};
  size_t m_size{};
};

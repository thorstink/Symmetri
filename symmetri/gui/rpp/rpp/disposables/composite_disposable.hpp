...
namespace rpp {
/**
 * @brief Disposable which can keep some other sub-disposables. When this root
 * disposable is disposed, then all sub-disposables would be disposed too.
...
class composite_disposable_impl : public interface_composite_disposable {
 public:
  ...
  composite_disposable_impl(composite_disposable_impl&& other) noexcept =
      delete;
  ...
  void dispose_impl(interface_disposable::Mode mode) noexcept final {
    while (true) {
      ...
      if (expected == State::Disposed) return;
    }
  }
  ...
  void add(disposable_wrapper disposable) override {
    if (disposable.is_disposed() || disposable.lock().get() == this) return;
  ...
  void remove(const disposable_wrapper& disposable) override {
    while (true) {
      State expected{State::None};
      // need to acquire possible disposables state changing from other `add` or
      // `remove`
      if (m_current_state.compare_exchange_strong(expected, State::Edit,
                                                  std::memory_order::seq_cst)) {
        try {
...
class composite_disposable
    : public composite_disposable_impl<
          rpp::details::disposables::default_disposables_container> {};
}  // namespace rpp

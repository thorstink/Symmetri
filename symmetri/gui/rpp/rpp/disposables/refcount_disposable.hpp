...
class refcount_disposable
    : public rpp::details::enable_wrapper_from_this<refcount_disposable>,
  ...
  void release() {
    ...
    while (current_value != s_disposed) {
      const size_t new_value =
          current_value == 1 ? s_disposed : current_value - 1;
      if (!m_refcount.compare_exchange_strong(current_value, new_value,
                                              std::memory_order::seq_cst))
        continue;

      if (new_value == s_disposed) dispose();
      return;
    }
  }
 ...
 public:
  ...
  enum class Mode : bool { WeakRefStrongSource, StrongRefRefSource };
...
namespace rpp::details {
class refocunt_disposable_inner final
    : public rpp::composite_disposable,
 ...
 public:
  ...
  void composite_dispose_impl(
      interface_disposable::Mode mode) noexcept override {
    if (mode != interface_disposable::Mode::Destroying)
      m_state.remove(this->wrapper_from_this());

    if (const auto locked = m_state.lock()) locked->release();
...
inline composite_disposable_wrapper refcount_disposable::add_ref(
  ...
  while (true) {
    ...
    if (m_refcount.compare_exchange_strong(current_value, current_value + 1,
                                           std::memory_order::seq_cst)) {
      auto inner = composite_disposable_wrapper::make<
          details::refocunt_disposable_inner>(
          mode == Mode::WeakRefStrongSource ? wrapper_from_this()
                                            : wrapper_from_this().as_weak());

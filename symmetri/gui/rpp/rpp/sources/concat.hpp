...
struct concat_state_t {
  concat_state_t(TObserver&& in_observer, const PackedContainer& in_container)
      : observer(std::move(in_observer)), container(in_container) {
...
struct concat_source_observer_strategy {
  ...
  bool is_disposed() const {
    return locally_disposed || state->disposable.is_disposed();
  }
...
template <rpp::constraint::observer TObserver, typename PackedContainer>
void drain(
    const std::shared_ptr<concat_state_t<TObserver, PackedContainer>>& state) {
  while (!state->disposable.is_disposed()) {
    ...
    using value_type = rpp::utils::extract_observable_type_t<
        utils::iterable_value_t<PackedContainer>>;
    state->is_inside_drain.store(true, std::memory_order::seq_cst);
    try {
      (*(state->itr.value()++))
          .subscribe(
              observer<value_type, concat_source_observer_strategy<
                                       std::decay_t<TObserver>,
                                       std::decay_t<PackedContainer>>>{state});
...
struct concat_strategy {
  template <typename... Args>
    requires(!constraint::variadic_decayed_same_as<
             concat_strategy<PackedContainer>, Args...>)
  concat_strategy(Args&&... args) : container{std::forward<Args>(args)...} {}
  ...
  void subscribe(observer<value_type, Strategy>&& obs) const {
    drain(std::make_shared<
          concat_state_t<observer<value_type, Strategy>, PackedContainer>>(
...
auto make_concat_from_iterable(Args&&... args) {
  return observable<utils::extract_observable_type_t<
                        utils::iterable_value_t<std::decay_t<PackedContainer>>>,
                    concat_strategy<std::decay_t<PackedContainer>>>{
...
auto concat(TObservable&& obs, TObservables&&... others) {
  if constexpr ((rpp::constraint::decayed_same_as<TObservable, TObservables> &&
    ...
    using inner_container =
        std::array<std::decay_t<TObservable>, sizeof...(TObservables) + 1>;
    using container = std::conditional_t<
        std::same_as<MemoryModel, rpp::memory_model::use_stack>,
        inner_container, details::shared_container<inner_container>>;
    return rpp::details::make_concat_from_iterable<container>(
        std::forward<TObservable>(obs), std::forward<TObservables>(others)...);
  } else {
    using variant_observable_t = rpp::variant_observable<
        rpp::utils::extract_observable_type_t<TObservable>,
        std::decay_t<TObservable>, std::decay_t<TObservables>...>;
    return concat<MemoryModel>(
        variant_observable_t{std::forward<TObservable>(obs)},
        variant_observable_t{std::forward<TObservables>(others)}...);
  }
}

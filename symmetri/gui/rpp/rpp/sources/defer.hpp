...
struct defer_strategy {
  using value_type =
      rpp::utils::extract_observable_type_t<std::invoke_result_t<Factory>>;
  using optimal_disposables_strategy =
      typename std::invoke_result_t<Factory>::optimal_disposables_strategy;
...
/**
 * @brief Creates rpp::observable that calls the specified observable factory to
 * create an observable for each new observer that subscribes.
 *
...
auto defer(Factory&& observable_factory) {
  return defer_observable<
      rpp::utils::extract_observable_type_t<std::invoke_result_t<Factory>>,
      Factory>{std::forward<Factory>(observable_factory)};
}
}  // namespace rpp::source

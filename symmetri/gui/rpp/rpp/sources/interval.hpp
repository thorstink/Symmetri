...
struct interval_schedulable {
  rpp::schedulers::optional_delay_from_this_timepoint operator()(
      const auto& observer, rpp::schedulers::duration period,
...
struct interval_strategy {
  ...
  void subscribe(TObs&& observer) const {
    ...
    worker.schedule(initial, interval_schedulable{},
                    std::forward<TObs>(observer), period, size_t{});
  }
};
}  // namespace rpp::details
...
template <schedulers::constraint::scheduler TScheduler,
          typename TimePointOrDuration>
using interval_observable =
    observable<size_t,
               details::interval_strategy<TScheduler, TimePointOrDuration>>;
}  // namespace rpp

namespace rpp::source {

/**
 * @brief Creates rpp::observable that emits a sequential integer every
 specified time interval, on the specified scheduler.
 *
...
auto interval(rpp::schedulers::duration initial,
  ...
  return interval_observable<std::decay_t<TScheduler>,
                             rpp::schedulers::duration>{
      std::forward<TScheduler>(scheduler), initial, period};
}

/**
 * @brief Same rpp::source::interval but using a time_point as initial time

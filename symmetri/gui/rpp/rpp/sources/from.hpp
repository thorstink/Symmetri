...
void iterate(const auto& iterable,
  ...
  if constexpr (constraint::decayed_same_as<decltype(scheduler),
    ...
    worker.schedule(
        [iterable = iterable,
         ...
         index = size_t{0}]() mutable -> schedulers::optional_duration {
          try {
            const auto& extracted_iterable =
                extract_iterable_from_packed(iterable);
...
template <memory_model memory_model, constraint::iterable Container,
          typename... Ts>
...
auto pack_variadic(Ts&&... items) {
  return pack_to_container<memory_model, std::array<T, sizeof...(Ts)>>(
...
class iterate_impl {
 public:
  iterate_impl(const PackedIterable& iterable, const TScheduler& scheduler)
      : m_iterable{iterable}, m_scheduler{scheduler} {}

  iterate_impl(PackedIterable&& iterable, const TScheduler& scheduler)
      : m_iterable{std::move(iterable)}, m_scheduler{scheduler} {}
...
/**
 * \brief Creates rpp::specific_observable that emits a particular items and
 completes
 *
 * \marble just
   {
 ...
 * \tparam Scheduler type of scheduler used for scheduling of submissions: next
...
{
  return create<std::decay_t<T>>(details::iterate_impl{
      details::pack_variadic<memory_model, std::decay_t<T>>(
          std::forward<T>(item), std::forward<Ts>(items)...),
      scheduler});
}
...
auto just(T&& item, Ts&&... items)
  requires(rpp::details::is_header_included<rpp::details::just_tag, T, Ts...> &&
           (constraint::decayed_same_as<T, Ts> && ...))
{
  return just<memory_model>(schedulers::trampoline{}, std::forward<T>(item),
                            std::forward<Ts>(items)...);
}

/**
 * \brief Creates rpp::specific_observable that emits a items from provided
 iterable
 *
 ...
 *
 * \tparam memory_model rpp::memory_model strategy used to handle provided
 iterable
 ...
 */
template <
    memory_model memory_model /* = memory_model::use_stack */,
  ...
  requires rpp::details::is_header_included<rpp::details::from_tag, TScheduler>
{
  ...
  return create<utils::iterable_value_t<Container>>(
      details::iterate_impl{details::pack_to_container<memory_model, Container>(
                                            ...
                                            decltype(callable)>
{
  ...
  if constexpr (std::same_as<utils::decayed_invoke_result_t<decltype(callable)>,
                             void>)
    return std::move(obs).map([](const auto& fn) {
      fn();
      return utils::none{};
    });
  else

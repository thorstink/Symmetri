...
class shared_container {
 public:
  template <typename... Ts>
    requires(!constraint::variadic_decayed_same_as<shared_container<Container>,
                                                   Ts...>)
...
struct from_iterable_schedulable {
  template <
      constraint::decayed_type PackedContainer,
      constraint::observer_strategy<utils::iterable_value_t<PackedContainer>>
          Strategy>
  rpp::schedulers::optional_delay_from_now operator()(
      const observer<utils::iterable_value_t<PackedContainer>, Strategy>& obs,
      const PackedContainer& cont, size_t& index) const {
    try {
...
struct from_iterable_strategy {
 public:
  ...
  using expected_disposable_strategy = std::conditional_t<
      rpp::schedulers::utils::get_worker_t<TScheduler>::is_none_disposable,
  ...
  template <
      constraint::observer_strategy<utils::iterable_value_t<PackedContainer>>
          Strategy>
  void subscribe(observer<utils::iterable_value_t<PackedContainer>, Strategy>&&
                     obs) const {
    if constexpr (std::same_as<TScheduler, schedulers::immediate>) {
      try {
        for (const auto& v : container) {
          if (obs.is_disposed()) return;
...
template <typename PackedContainer,
          schedulers::constraint::scheduler TScheduler, typename... Args>
auto make_from_iterable_observable(const TScheduler& scheduler,
                                   Args&&... args) {
  return observable<utils::iterable_value_t<std::decay_t<PackedContainer>>,
                    details::from_iterable_strategy<
                        std::decay_t<PackedContainer>, TScheduler>>{
...
struct from_callable_invoke {
  ...
  auto operator()(Callable&& fn) const {
    if constexpr (std::same_as<utils::decayed_invoke_result_t<Callable>,
                               void>) {
      fn();
 ...
 *
 * @tparam memory_model rpp::memory_model strategy used to handle provided
 iterable
 * @param scheduler is scheduler used for scheduling of submissions: next item
 will be submitted to scheduler when previous one is executed
...
template <constraint::memory_model MemoryModel /* = memory_model::use_stack*/,
          ...
          schedulers::constraint::scheduler
              TScheduler /* = rpp::schedulers::defaults::iteration_scheduler*/>
auto from_iterable(Iterable&& iterable,
                   const TScheduler& scheduler /* = TScheduler{}*/) {
  using container = std::conditional_t<
      std::same_as<MemoryModel, rpp::memory_model::use_stack>,
      std::decay_t<Iterable>,
      details::shared_container<std::decay_t<Iterable>>>;
  return details::make_from_iterable_observable<container>(
      scheduler, std::forward<Iterable>(iterable));
}
 ...
 * @tparam Scheduler type of scheduler used for scheduling of submissions: next
 item will be submitted to scheduler when previous one is executed
...
auto just(const TScheduler& scheduler, T&& item, Ts&&... items) {
  ...
  using container = std::conditional_t<
      std::same_as<MemoryModel, rpp::memory_model::use_stack>, inner_container,
      details::shared_container<inner_container>>;
  return details::make_from_iterable_observable<container>(
      scheduler, std::forward<T>(item), std::forward<Ts>(items)...);
}
...
auto just(T&& item, Ts&&... items) {
  ...
  using container = std::conditional_t<
      ...
      details::shared_container<inner_container>>;
  return details::make_from_iterable_observable<container>(
      rpp::schedulers::defaults::iteration_scheduler{}, std::forward<T>(item),
      std::forward<Ts>(items)...);
}

/**
 * @brief Creates rpp::specific_observable that calls provided callable and
 emits resulting value of this callable
 *
...
auto from_callable(Callable&& callable) {
  return just<MemoryModel>(std::forward<Callable>(callable)) |
         rpp::operators::map(details::from_callable_invoke{});
}
}  // namespace rpp::source

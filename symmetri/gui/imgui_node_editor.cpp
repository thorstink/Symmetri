...
void ed::EditorContext::Begin(const char* id, const ImVec2& size) {
  ...
  if (!m_IsInitialized) {
    ...
    // are loaded
    if (m_Canvas.Begin(id, canvasSize)) m_Canvas.End();
...
void ed::EditorContext::End() {
  ...
  for (auto node : m_Nodes)
    if (node->m_IsLive && node->IsVisible()) node->Draw(m_DrawList);

  // Draw links
  for (auto link : m_Links)
    if (link->m_IsLive && link->IsVisible()) link->Draw(m_DrawList);
  ...
  // Draw animations
  for (auto controller : m_AnimationControllers) controller->Draw(m_DrawList);
...
void ed::EditorContext::UpdateNodeState(Node* node) {
  ...
  if (!tryLoadState && settings->m_WasUsed) return;
...
void ed::EditorContext::ClearSelection() {
  for (auto& object : m_SelectedObjects) object->m_IsSelected = false;
...
void ed::EditorContext::DeselectObject(Object* object) {
  ...
  if (objectIt == m_SelectedObjects.end()) return;
...
int ed::EditorContext::BreakLinks(NodeId nodeId) {
  ...
  for (auto link : m_Links) {
    ...
    if (link->m_StartPin->m_Node->m_ID == nodeId ||
        link->m_EndPin->m_Node->m_ID == nodeId) {
      if (GetItemDeleter().Add(link)) ++result;
    }
  }
  return result;
}
...
void ed::EditorContext::Suspend(SuspendFlags flags) {
  ...
  if (m_IsCanvasVisible) m_Canvas.Suspend();
...
void ed::EditorContext::Resume(SuspendFlags flags) {
  ...
  auto lastChannel = m_DrawList->_Splitter._Current;
  m_DrawList->ChannelsSetCurrent(m_ExternalChannel);
  if (m_IsCanvasVisible) m_Canvas.Resume();
  m_DrawList->ChannelsSetCurrent(lastChannel);
}
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen) {
  ...
  if (!allowOffscreen && !windowHovered && !widgetHovered) return Control();
  ...
  static auto invisibleButtonEx = [](const char* str_id, const ImVec2& size_arg,
    ...
    if (size_arg.x == 0.0f || size_arg.y == 0.0f) return false;

    const ImGuiID id = window->GetID(str_id);
    ImVec2 size = CalcItemSize(size_arg, 0.0f, 0.0f);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ItemSize(size);
    if (!ItemAdd(bb, id)) return -1;
  ...
  auto emitInteractiveAreaEx = [&activeId](ObjectId id, const ImRect& rect,
    ...
    if (ImGui::IsItemActive()) activeId = ImGui::GetActiveID();
  ...
  auto checkInteractionsInArea =
      [this, &emitInteractiveArea, &hotObject, &activeObject, &clickedObject,
       &doubleClickedObject](ObjectId id, const ImRect& rect, Object* object) {
        if (emitInteractiveArea(id, rect) >= 0) clickedObject = object;
  ...
  auto isMouseDoubleClickOverBackground = [doubleClickedObject,
    ...
    if (doubleClickedObject) return -1;

    if (!ImGui::IsItemHovered()) return -1;
  ...
  if (backgroundDoubleClickButtonIndex >= 0) backgroundClickButonIndex = -1;
  ...
  if (activeId) m_EditorActiveId = activeId;
  ...
  if (!allowOffscreen && !m_IsHovered) return Control();

#if IMGUI_VERSION_NUM >= 18836
  if (m_IsHoveredWithoutOverlapp) ImGui::SetItemKeyOwner(ImGuiKey_MouseWheelY);
#elif IMGUI_VERSION_NUM >= 17909
  if (m_IsHoveredWithoutOverlapp) ImGui::SetItemUsingMouseWheel();
#endif
...
void ed::FlowAnimationController::Flow(Link* link, FlowDirection direction) {
  ...
  if (direction == FlowDirection::Backward) speedDirection = -1.0f;
...
int ed::NavigateAction::MatchZoomIndex(int direction) {
  ...
  if (bestDistance > 0.001f) {
    if (direction > 0) {
      ...
      if (bestIndex >= m_ZoomLevelCount) bestIndex = m_ZoomLevelCount - 1;
    } else if (direction < 0) {
      --bestIndex;
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(
    const Control& control) {
  if (!Editor->IsFocused() || !Editor->AreShortcutsEnabled()) return False;
...
void ed::DeleteItemsAction::DeleteDeadLinks(NodeId nodeId) {
  ...
  for (auto link : links) {
    ...
    if (it != m_CandidateObjects.end()) continue;

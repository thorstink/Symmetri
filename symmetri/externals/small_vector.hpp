...
concept AllocatorFor =
    ...
    requires(A a, typename std::allocator_traits<A>::template rebind_alloc<U> b,
             A a1, A a2, U xp, typename std::allocator_traits<A>::pointer p,
      ...
      requires LegacyContiguousIterator<
          typename std::allocator_traits<A>::pointer>;
...
class GCH_EMPTY_BASE allocator_interface : public allocator_inliner<Allocator> {
 ...
 public:
  ...
  //    allocator_interface (const allocator_interface&)                     =
  //    impl;
  ...
  maybe_copy(const allocator_interface& other) noexcept(
  ...
  GCH_NODISCARD
  static GCH_CPP17_CONSTEXPR size_ty internal_range_length(
  ...
  GCH_NODISCARD GCH_CPP20_CONSTEXPR size_ty get_max_size(void) const noexcept {
    ...
    return (
        std::min)(static_cast<size_ty>(alloc_traits::max_size(allocator_ref())),
  ...
  template <typename A = alloc_ty, typename V = value_ty, typename... Args,
            typename std::enable_if<must_use_alloc_construct<
                A, V, Args...>::value>::type* = nullptr>
  ...
  template <typename A = alloc_ty, typename V = value_ty, typename U,
            typename std::enable_if<
                !must_use_alloc_construct<A, V, U>::value &&
...
class small_vector_base : public allocator_interface<Allocator> {
...
#else
                                    small_vector_adl::is_nothrow_swappable<
                                        T>::value
#endif
                                    > {
  };
 ...
 private:
  ...
  void set_capacity(size_ty capacity) noexcept {
    assert(InlineCapacity <= capacity &&
           "capacity must be greater than InlineCapacity.");
 ...
 protected:
  GCH_NODISCARD GCH_CPP14_CONSTEXPR size_ty calculate_new_capacity(
      const size_ty current, const size_ty required) const noexcept {
    ...
    // it falls flat: size_ty new_capacity = current + (current / 2);
  ...
  template <typename ForwardIt>
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements(const ForwardIt first,
                                                       const ForwardIt last,
                                                       const size_ty count) {
    // The `count` parameter is just an optimization, since it may already be
    // calculated by the parent. This is important because it may be O(n) to
  ...
  template <
      ...
      typename std::enable_if<is_nothrow_emplace_insertable<
          typename std::iterator_traits<ForwardIt>::reference>::value>::type* =
          nullptr>
  ...
  template <
      ...
      typename std::enable_if<!is_nothrow_emplace_insertable<
          ...
          nullptr>
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements_with_allocator(
      ForwardIt first, ForwardIt last, alloc_interface& alloc) {
    ptr l_pivot;
    ForwardIt r_pivot;
    ...
    GCH_CATCH(...) {
      alloc.destroy_range(unchecked_next(l_ptr),
                          unchecked_next(begin_ptr(), range_size));
  ...
  template <unsigned N, typename A = alloc_ty,
            typename std::enable_if<allocators_always_equal<A>::value>::type* =
                nullptr>
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements(
  ...
  template <typename ForwardIt>
  GCH_CPP20_CONSTEXPR small_vector_base& assign_with_range_with_allocator(
      ForwardIt first, ForwardIt last, alloc_interface& alloc) {
    size_ty count = internal_range_length(first, last);
    if (InlineCapacity < count) {
      const size_ty new_capacity =
          calculate_new_capacity(InlineCapacity, count);
  ...
  copy_assign_equal_or_non_propagated_allocators(
      const small_vector_base<Allocator, N>& other) {
    assign_with_range(other.begin_ptr(), other.end_ptr(),
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base&
  copy_assign_unequal_and_propagated_allocators(
      const small_vector_base<Allocator, N>& other) {
    // Note: We have to create a new alloc_interface here because `other` is
    // const.
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base&
  move_assign_equal_or_non_propagated_allocators(
      small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base&
  move_assign_unequal_and_propagated_allocators(
      small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N = InlineCapacity, typename A = alloc_ty,
            typename std::enable_if<
                N == 0 &&
                std::allocator_traits<
                    A>::propagate_on_container_move_assignment::value>::type* =
                nullptr>
  ...
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_unequal_allocators(
      small_vector_base<Allocator, N>& other) {
    // We cannot move an allocation pointer in this case, so go directly to the
    // inner function.
    return move_assign_equal_or_non_propagated_allocators(other);
  }
 ...
 public:
  ...
  GCH_CPP20_CONSTEXPR small_vector_base(
      bypass_tag, const small_vector_base<Allocator, I>& other, const A& alloc)
  ...
  GCH_CPP20_CONSTEXPR small_vector_base(
      ...
      : small_vector_base(bypass, other,
                          static_cast<const alloc_interface&>(other)) {}
 ...
 protected:
  ...
  GCH_CPP20_CONSTEXPR void assign_with_range(const ForwardIt first,
                                             const ForwardIt last,
                                             std::forward_iterator_tag) {
  ...
  GCH_CPP20_CONSTEXPR void swap_equal_or_propagated_allocators(
    ...
    if (has_allocation()) {
      if (InlineCapacity < other.get_capacity()) {
        // Note: This is always the branch that will run when
        // constant-evaluated.
        ...
        other.uninitialized_move(other.begin_ptr(), other.end_ptr(),
                                 new_data_ptr);
      ...
      // This implies that `other` is allocated, and that we can use its
      // pointer.

      size_ty new_capacity = LessEqualI;
      ptr new_data_ptr = other.storage_ptr();

      // Check to see if we can store our elements in the inline storage of
      // `other`.
      if (LessEqualI < get_size()) {
        ...
        new_data_ptr = alloc_interface::allocate_with_hint(
            new_capacity, allocation_end_ptr());
      ...
      size_ty new_capacity =
          other.calculate_new_capacity(LessEqualI, get_size());
  ...
  template <
      unsigned GreaterI, typename A = alloc_ty,
  ...
  GCH_CPP20_CONSTEXPR ptr copy_range(InputIt first, InputIt last, ptr dest) {
    ...
    if GCH_IF_CONSTEXPR (!std::is_same<decltype(unmove_iterator(first)),
                                       InputIt>::value)
      if (std::is_constant_evaluated())
 ...
 public:
  ...
  template <typename Generator,
            typename std::enable_if<
                ...
                !std::is_convertible<Generator, const allocator_type&>::value>::
                type* = nullptr>
#endif
...
inline GCH_CPP20_CONSTEXPR void swap(
    ...
    small_vector<T, InlineCapacityRHS, Allocator>&
        rhs) noexcept(noexcept(lhs.swap(rhs)))
#ifdef GCH_LIB_CONCEPTS
  requires concepts::MoveInsertable<
               T, small_vector<T, InlineCapacityLHS, Allocator>, Allocator> &&
           concepts::MoveAssignable<T> && concepts::Swappable<T>
#endif
{
  lhs.swap(rhs);
